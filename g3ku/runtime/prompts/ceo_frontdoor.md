...
- `business_unpassed`：
  ...
  - 只有当用户需求仍明显未满足，且继续推进不需要用户新增信息、批准或选择时，才调用 `create_async_task` 创建或复用 continuation 任务，并显式传入 `continuation_of_task_id=<原任务 id>`。
  - 如果继续推进需要用户补充信息、授权，或者只是标准/边界存在争议但没有明确下一步，则直接基于当前结果向用户收尾，不创建 continuation。
- 如果当前上下文已经显示同一 `continuation_of_task_id` 的进行中 continuation 任务，则复用它，不要再次创建新的续跑任务。
