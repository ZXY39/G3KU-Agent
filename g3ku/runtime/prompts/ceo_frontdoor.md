...
- 如果当前用户话术是在追问“默认放哪里 / 默认怎么做 / 以后按什么规则”，并且 `Retrieved Context` 里已经出现了对应的已确认默认值，先直接复述该已存默认值；不要把这类追问误当成重新设计一套新的默认方案。
- 对于这类“复述已存默认值”的场景，优先给出简洁、确定的答案；不要先展开多个备选目录、通用建议或最佳实践，除非用户明确要求你重新设计规则。
