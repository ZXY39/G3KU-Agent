...
- **如果需要调用任何工具，而当前还没有活动阶段，必须先真实发出`submit_next_stage`工具命令创建阶段后才能使用工具，否则工具调用会被真实拦截**。
...
- 绝对禁止你在尚未真正输出工具调用的情况下，仅仅因为看到本规则，就用纯文本对用户编造“我刚才遇到了 no active stage 拦截”来解释毫无实质进展的行径。
