...
- 调用 `create_async_task`（创建异步任务）时，任务说明必须写清目标范围、关键线索和目标产出，避免无边界描述或模糊候选列表。
...
- 凡是有关安装/更新/上网搜索/下载 skill 的这类请求，必须先 `load_skill_context(skill_id="clawhub-skill-manager")`，并把它作为唯一工作流入口，不要直接把 `skill-creator` 当作 skill 下载器。
