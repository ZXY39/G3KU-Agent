...
- 验收/节点把合法 PDF（或图片、xlsx 等二进制交付物）判成几十字节空壳、或在其上搜索 `%PDF` 等签名 0 命中 → `tool-and-skill-system.md`「外置工具结果信封」（二进制目标的展示契约与字节级搜索）
...
- 日志审计侧栏角标不更新、异常板块恒隐藏、原始日志为空、`/api/audit` 503 → `web-and-admin.md`「Log Audit Page And Event Contract」
