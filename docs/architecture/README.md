...
- False "task may be stalled" heartbeat while a long node tool (e.g. `exec` with a large `timeout`) legitimately runs, or a genuine hang after a tool timeout goes unreported -> `heartbeat-system.md`「Task Stall Detection」
...
- 任务树过大打不开（`Failed to open task: Request timeout`）、打开时长时间无树或「加载中(x/xx)」进度异常 → `web-and-admin.md`「Task Tree Chunked Load Contract」
