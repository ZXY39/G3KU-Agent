...
Execution/final-acceptance now also use a shared reflation handshake. If you are debugging “execution succeeded but vanished from the parent tree”, “why is an acceptance node visible before activation?”, or “why did final acceptance reject once and the task kept running?”, read `runtime-overview.md`, `web-and-admin.md`, and `operations-and-maintenance.md` together before changing task-tree projection or terminal-task handling.
