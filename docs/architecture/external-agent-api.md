...
- 消息提交成功但桥收不到回复：先确认 pump 是否真的在跑——锚点 `qq-official pump connecting session=<会话键> last_seq=N`，**每次连接尝试一条 INFO**。它在、而 `qq-official delivered` 不在 ⇒ 事件没到或投递被判失败（找桥侧 WARNING）；它一条都没有 ⇒ pump 从未被执行（任务没被调度/桥没建 pump），此时 18790 上会只剩浏览器 WS 那条回环长连接，`outbox.jsonl` 也是空的——这是"入站正常、出站黑洞"的最小心证。再确认桥订阅的 SSE 会话与消息提交的会话一致（同一 `session_id`）；看事件缓冲是否被 `eventBufferSize` 淘汰（长断线超过缓冲窗口）。
