...
- `submit_next_stage` 另有一个可选布尔参数 `drop_completed_stage_tool_detail`（三条路径同一个 `SubmitNextStageTool`，schema 与校验只有一份）：为真时把**正在关闭**的那条阶段的工具肉身移出 provider 上下文，只留下阶段块里的总结。它要求同批带非空 `completed_stage_summary`，否则 `validate_params` 判参数非法走既有的参数错误回贴车道——库里 46.2% 的终态阶段总结本就为空，没总结就裁等于移走该阶段唯一的记录。提交落点还会再收一次（`drop and summary` 同时成立才写标记），所以绕过工具层的写入者造不出黑洞态。它与收口是两个不同的账本字段、两种信息损失边界，机制与实测见 `runtime-overview.md`「stage_compaction」。
- `read_completed_stage` 是前门专用的**内置合同工具**（`main/runtime/internal_tools.py` 的 `ReadCompletedStageTool`，与 `silent` 同一族），不是 `tools/*/resource.yaml` 声明的资源包工具：它按 `stage_indexes`（上限 5）读回本会话已被移出上下文的阶段，账本从图状态闭包取，参数只能挑读哪几条、挑不了读谁的会话，所以不需要 `allowed_roles` 面。接线四点，缺一个工具就存在但看不见、或看得见但被闸门拦：注册进 `_ceo_runtime_ops` 的 `all_tools`、`_frontdoor_callable_tool_names_for_state` 与 `CeoMessageBuilder.ALWAYS_CALLABLE_INTERNAL_TOOLS`（后者才是不依赖可见集/水化的注入点）、`stage_budget` 的闸门豁免与不占预算两个集合（不要放进 `CONTEXT_LOADER_STAGE_TOOL_NAMES`——那族会同时灌进节点的记账豁免集和"不占预算工具"清单，把前门专有名字说成节点能调的）。返回逐条带 `arguments_text`，`output_text` **只在存在 `output_ref` 时**才截到 8000：写入侧允许内联到 8K+ 且不留指针（实测 554 阶段的会话里 68 个如此），对没有指针的正文动刀就是永久丢内容。每条都回带 `stage_goal` 与 `created_at`，因为 `stage_index` 会随 rebase 重编（实测见过 12010）。常驻代价：模型可见 schema 314 字符 ≈ 105 tok/请求（`base.py` 发的是 `model_description` + `model_parameters`，不是完整 `description`），同族参照 `silent` 149 tok、`submit_next_stage` 333 tok。
