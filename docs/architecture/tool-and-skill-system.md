...
- 兜底闸门的两处豁免：① 落库类工具 `memory_write` / `memory_delete` / `memory_note` 是 CEO/frontdoor 全局白名单（`FRONTDOOR_STAGELESS_MEMORY_TOOL_NAMES`），无活动阶段也可调用——保证"用户口头给一条长期指令 → 写记忆"不会被 `no active stage` 拦下后一次性放弃、永久丢失；② 节点暂停（`task_node_error`）心跳轮 `allow_stageless` 放行首个实质性工具，阶段由 `_frontdoor_stage_state_after_tool_cycle` 自动补开（`system_generated`，预算 10，标题=「任务 ID xxx 中的节点出现自动暂停，检查原因并处理」），预算耗尽后 `transition_required` 置真、回到普通闸门。这两处豁免只改执行门控，不改变节点快照 / CEO 合同的 callable 列表，也不改 provider `tools[]` 前缀稳定性。
