...
| `filesystem_edit` | 模型可见契约全平面、不含 object 字段：必填 `path + new_text`，区域由**二者之一**定位——`old_text`（逐字节匹配、须唯一，用相邻行扩到唯一为止），或 `start_line` + `end_line`（1-based 闭区间，取自对当前文件的读取，`new_text` 直接承载替换内容、不必复述旧文本）。没有 `by` / `mode` 判别器，也不需要在 schema 里表达互斥。`new_text: ""` 即删除该区域。 | 行号车道不会大声失败：陈旧或错一位的区间照样替换成功，所以成功文本回显被消费的区域（`replaced lines 41-58, first was: ...`），让模型看得见自己替换的是哪一段；`old_text` 车道回显 `resolved lines N-M`。**同一文件的一轮里不得并发多条区间编辑**——每条都会移动其余条目的基准行号；`old_text` 车道对位移免疫。幂等覆盖两条平面车道：`old_text` 已不在而 `new_text` 唯一在盘，或给定行区间的内容已等于 `new_text` 时返回 `Already applied:` 且不写文件（与 `main/runtime/recovery_check.py` 判定“结果丢失但磁盘已生效”同判据）；删除车道（`new_text` 为空）无此判据，重放会二次替换。两个定位器同时给、或都不给，都按错误打回且不写盘。校验面 `parameters` 有意宽于模型面：`target.by` ∈ `exact_text` / `anchor_pair` / `line_range` 与 `mode + ...` 仍是可执行车道（历史转录与 adapter 形状继续可跑），但不再向模型曝光——模型面没有 object，就不存在“把替换文本塞进定位器对象”这个错法；判定 legacy 类别前先剥离对侧车道的占位值（`start_line=0` / `replacement=""`）吸收自动填充噪声。模型面不出现 object 字段的根因：provider schema 管线无法表达互斥与条件必填（组合关键字会被压平）。`load_tool_context` / `get_tool_toolskill` 的参数摘要只渲染模型面，且示例只由必填字段构造，因此两种入参形状必须由 toolskill 正文给出。 |
