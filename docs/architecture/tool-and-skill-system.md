...
- 轮末若模型直接输出文本而不再开阶段、且仍有 `pending_orphan_rounds`，finalize 会自动补开一个 `system_generated` 阶段收纳这些孤儿轮（`stage_kind` 仍是 `normal`，保证被阶段压缩正常回收；`stage_goal` 派生自当轮叙述）。轮末收尾不再把回复全文写进 `completed_stage_summary`，改为写入一行指针（`STAGE_TURN_END_SUMMARY_POINTER`）——全文仍以 assistant 消息原件保留在对话历史里；阶段压缩只删工具肉身、原样保留纯文本对话，若全文入账，阶段过期压缩后同文会在上下文出现两份。
- 当前没有「有效阶段」（含预算耗尽、必须换阶段）时，CEO/frontdoor 的 agent-facing `frontdoor_runtime_tool_contract.callable_tool_names` 不再收紧到只剩 `submit_next_stage`，而是保留全量 callable 并把 `submit_next_stage` 置首，配合同批提交协议与执行期宽限兜底；execution / acceptance 节点仍收紧到只剩 `submit_next_stage`。任何阶段都不收紧 provider body 里的 `tools[]`：为保持 prompt cache 前缀稳定，provider-facing 继续使用稳定的 runtime-visible tool bundle，阶段控制交给动态合同与执行门控（详见下文「CEO Provider Tool Surface」）。
- execution / acceptance 节点不必把 `submit_next_stage` 单独拆成一轮：阶段切换成功时，同批普通工具作为新阶段首轮执行；切换失败时，同批剩余普通工具被批内阻断，不回退旧阶段继续执行。执行层的 `stage_gate_error_for_tool()` 是 schema 收紧与提示协议之外的兜底防线：节点侧模型仍尝试普通工具时返回 `no active stage` / `current stage budget is exhausted`；CEO/frontdoor 侧在宽限用尽后由同一条兜底闸门拦截。同一批内多次提交 `submit_next_stage` 只执行第一次，其余返回「至多一次」错误。
...
- 前门要区分两份工具集合：`tool_names` 保存阶段内可恢复的完整 callable pool；“当前轮合同暴露给模型的 callable tools”要通过前门 callable-tool helper 结合 `frontdoor_stage_state` 再算一次。不要把前者直接当作当前轮模型可见函数列表；`submit_next_stage` 的置首与节点侧 callable 收紧只影响 agent-facing 合同，provider `tools[]` 始终不同步收紧。
