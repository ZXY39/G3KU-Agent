...
- This matters especially when `exec` output is long. The current shell payload normally externalizes long output down to `head_preview`, not stable `stdout_ref` / `stderr_ref` handles in ordinary agent-facing results. If a node keeps trying to extract source snippets through repeated `exec`, fix prompt/runtime guidance toward `content_open(path)` rather than teaching the model to expect hidden full-output refs.
