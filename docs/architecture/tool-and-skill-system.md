...
For CEO/frontdoor prompt-cache debugging, maintainers distinguish two tool surfaces: `tool_names` represent the current turn's agent-facing callable pool, while `provider_tool_names` represent the provider-facing stable superset used to build `tools[]` for function calling. Hydration promotion and stage gating change the tail `frontdoor_runtime_tool_contract`, but must not churn the provider-facing `tools[]` bundle every round.
