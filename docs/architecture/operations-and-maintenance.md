...
- 冻结/复活/收尸类异常在 worker 日志有固定签名，先 grep 再看 DB：`node frozen by distribution hold`（hold 冻结落点，带 task/node/epoch）、`stale subtree hold ignored`（meta 与 epochs 表脱同步被防御放行）、`release verification`（释放后节点未复活，先 WARN 再 resume、仍卡死落 ERROR）、`dispatch future was cancelled while node non-terminal` / `stranded exception`（搁浅 future 被释放路径重建）、`orphan node reaped` / `orphan node re-dispatched`（run_task 入口对孤儿子节点的决断，收尸同时写 `task_error_logs`）。子节点"显示进行中但无模型调用/无帧更新"时按此顺序核对。
