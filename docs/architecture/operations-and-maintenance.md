...
- If operators report that a child "disappeared" from the parent tree right after successful execution, inspect the tree snapshot fields `parent_visible` and `acceptance_handshake_state` before assuming a projection bug. Execution nodes in `waiting_acceptance` are now intentionally hidden from the parent-facing tree while their unfinished acceptance nodes remain visible.
...
- If an acceptance loop looks stuck after a rejection, inspect execution-node `metadata.acceptance_handshake` together with task-level `final_acceptance`. The expected sequence is `waiting_acceptance -> waiting_execution_retry -> accepted|rejected_terminal`, with at most two rejection outcomes; execution failure during the retry should end as `canceled_by_execution_failure`, not as another acceptance pass.
