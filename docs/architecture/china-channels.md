...
- 只发送最终对用户可见的文本
- 所有 `deliver_message` 帧在 `build_deliver_frame(...)` 统一清洗出站文本（`sanitize_channel_outbound_text`）：移除 standalone `frontdoor_runtime_tool_contract` 回显及可见文本后的契约尾段，并截断 `[SESSION EVENTS]` 之后的内容。清洗后为空说明整条消息都是内部文本，直接跳过投递——`send_outbound(...)` 对此正常返回（drain 视为已送达并 ack），不抛 `RuntimeError`（那会触发重试风暴）。
