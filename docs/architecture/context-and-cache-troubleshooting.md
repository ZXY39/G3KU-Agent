...
- 不变量：当前 turn 已有真实 provider request 落盘后，`finalize` 阶段产出的可见最终回复必须补回 session-owned baseline；该规则不限于 `direct_reply`，普通工具（如 `message`）之后产出的用户可见 `final_output` 同样要补回。该规则同样覆盖 heartbeat/cron 内部回合的可见回复：补回与否按回复内容判定，不看回合类型；调用 `silent` 的回合豁免——它的正文已经随带 `tool_calls` 的那行助手记录进了基线，再补一次就是同文两份。“同 turn 已有 actual-request 证据”与“fresh turn 没有 actual-request 证据”是两种不同状态，不能用同一条覆盖规则处理。
...
### 3.11 残留 paused / pending 转录条目与未回答的用户输入
...
- 坑（失败回合）：运行时错误收尾写的助手行只证明"这一轮跑坏了"，不证明"用户被回答了"，但它带 `metadata.source=runtime_error`。把这种行算进"已回答"集合，等于把失败回合的用户输入永久锁在接回之外；同一条输入若又被按已消费翻成 `completed`，它就既不在内存队列、也不在待接回名单里，模型上下文里彻底消失。用户接着发"继续"，模型答的是上一件成功的事——这条与上面两条是同一族缺陷的相反方向（重复投喂 vs 永久缺席）。
- 不变量：残留 paused 条目随下一个用户可见回合正常完成被对账退役一次；pending 行随**任何**正常完成回合（含内部回合）按"内存队列不再持有它"退役一次。运行时错误路径既不退役也不升格，并把未获真实模型轮次的输入放回内存队列；状态机、为什么"队列才是真相"、以及哪条车道认领待重投输入，归 `runtime-overview.md`「压缩窗口：入站闸门与基线写入仲裁」。退役与对账在持久化侧的完整生命周期见「Baseline 合同与恢复顺序」。
- 症状：连续多轮 actual request artifact 里同一条历史用户消息反复出现在请求体尾部，紧邻当前轮用户消息且前后都没有助手回复；模型重复处理或批量补答早已回答过的历史问题，甚至把多条这样的消息合并成一条虚构的用户请求。渠道/Web 会话里另一副面孔是旧提问冒到最新回复下面，读起来像用户重发。反向症状是某轮失败后用户补一句"继续/刚才的问题"，模型回的内容与失败那轮无关。
- 取证：日志锚点 `Rehydrated N queued follow-up message(s) for <session>`、`Requeued N unanswered user message(s) for <session> after turn failure` 与 `Retired N consumed pending transcript user message(s) for <session>`——前两个在健康系统里应当罕见且 N 很小，长期不为 0 说明退役没有发生。转录侧直接数 `role=user` 且 `metadata._transcript_state=pending` 的行，跨天存在即为漏网；再数 `role=assistant` 且 `metadata.source=runtime_error` 的行，逐个核它前面那条用户行的状态是否仍是 `pending`（成了 `completed` 就是这条被吃掉的路径）。
...
- 负载均衡器把一个节点在其生命周期内粘滞绑在一个成员上，是**缓存命中契约的一部分**：换 `model_key` 等于换前缀缓存命名空间，断点之后的整段存活上下文都要重传，所以代价按「断点尾部存活量」计而不是按切换次数计。判读命中掉底先看该节点的 `resolved_model_key` 是否跨越了成员边界，再看重绑原因（过滤条件变化 / 惩罚超阈 / 拿不到 permit / 成员被移出组，见 `runtime-overview.md`「节点模型路由与准入绑定」）。阶段推进不在原因列表里：绑定跨阶段保留。
...
- 判读一次 `stage_compaction` 断点的代价，量的是**断点之后还剩多少要重传**，不是断点出现了几次。断点位置由"最老的仍带肉身阶段"的首帧决定，钉在头部附近且**不随保留窗口移动**（实测活节点在 index 19，其前 19 帧 = system + 引导 user + 已裁阶段的块 ≈ 3.74 万字符 ≈ 1.25 万 tok，与裁后那一跳的 `cache_hit` 1.69 万 tok 同量级）；变化的是它后面的载荷。窗口 3 时那里压着三个完整阶段（27.4 万字符 ≈ 9.2 万 tok），那一跳非缓存 `input` 从正常的 1.5–3 千冲到 7.4 万；模型点名裁撤（机制见 `runtime-overview.md`「stage_compaction」）时断点后面只剩刚开的 0–2 轮，同一跳的 `input` 回到千级。所以"每阶段都断一次"不比"攒三次断一次"贵：断点次数只由阶段数决定，单价与被裁体积成正比。预期读数：裁撤生效后，边界间正文天花板 ≈ 头部 + 一个阶段，非断跳 `effective_input` 从 9–12 万 tok 落到 2–4 万 tok。
- 正文尺寸有一个任何压缩都拿不掉的地板，读"压缩没生效"前先扣掉它：call id 不属于任何阶段 `rounds` 的帧永不进入过期集合。实测一条活节点 169 帧里 17 帧 / 2.19 万字符 = **正文的 7.05%**。构成是 `submit_final_result` 的被拒轮、单独提交（未与普通工具同批）的 `submit_next_stage` 边界轮、以及跨轮重复熔断的 error 轮——前两类在两条车道上都不记轮次（`react_loop.py` 的终态分派与熔断路径都在记账点之前 `continue`），属记账豁免不是丢账；配对完整性已全库验过（3,446 条无归属 tool 帧，0 条缺 `tool_call_id`、0 条找不到声明它的 assistant 帧），所以按 call id 裁帧的机制不受威胁。在跑的 126 个节点合计只有 24 帧 / 8.2 万字符落在这块地板上，量级不值得为它加回收判据。
- 终态提交反复被拒、每次错误文本首行相同但 arguments 后缀不同，先怀疑 provider 而不是模型。网关把参数块未转义拼进 `arguments` 时，客户端的宽容解析（`streaming_timeouts.py` / `openai_chat_provider.py` 优先 `json_repair`）会把闭合标记吸进第一个字符串值并吃掉后面的必填项，于是"解析故障"长成"模型漏填参数"。识别指纹是字符串值以半截标记收尾（残缺写法实测三种，按形态判不能穷举字符串）；运行时有独立计数器承接它，**不占**模型侧的无效提交预算，所以症状表现为反复收到协议提示后自行恢复，而不是节点被判死。这类帧会留在上面那块地板上（每链 4–8 帧），所以它同时也是正文里唯一值得清的重试残骸。
...
- `[G3KU_STAGE_COMPACT_V1]` — `stage_compaction` 生成的普通完成阶段块（工具肉身已剪掉）。请求体里的块数量等于"未收口的完成阶段数 − raw 保留窗口"，因此一次带收口的压缩之后它应当塌到个位数；数量不降见「Shrink 原因与压缩边界」的收口判读。元数据只写非默认/非空字段：`stage_index` 恒有（块位置记忆与存量块去重都依赖它，缺失会导致块跳位），`stage_goal`、`completed_stage_summary`、`key_refs`、`tool_rounds`（`已用/预算`）、`mode`（仅非默认 `自主执行` 时写）、`system_generated`（仅 true 时写）、`evicted`（仅 true 时写＝该阶段的工具肉身是模型自己点名移出的，不是当时就没记）、`archive_ref`（仅裁撤导档成功时写，指向该阶段全量账本文件，用 `content_open` 打开；没有这行就是没导出来，不要猜路径）——读回通道见 `runtime-overview.md`「stage_compaction」：节点走 `task_node_detail`，前门走块里的 `archive_ref`；本分支恒为普通完成阶段，故 `stage_kind` / `status` 不写。摘要为空是正常形态——该阶段的最终回复作为 assistant 消息紧随块之后，块不复述结论。不携带工具明细是有损设计的预期，不是数据缺失。
...
节点阶段压缩是否生效，看**过期点那一跳的发送体是否塌下来**：`prepared_message_chars` / `observed_input_truth.effective_input_tokens` 应在某个阶段把另一个阶段挤出保留窗的那一跳台阶式下降，同轮 `request_seed_source=same_turn_stage_compacted`、`history_shrink_reason=stage_compaction`。除此之外同回合链仍是字面前缀增长，所以曲线是"平台 + 周期性台阶"，不是逐轮下降；`model_message_chars`（投影）与发送体的差只在过期点那一跳被抹平，其余轮次投影更早就等于发送体。判据反过来的情形才是回归：节点账本已有 >3 个终态阶段、`model_message_chars` 与 `prepared_message_chars` 却逐轮同规模同涨，说明压缩又变成"只加摘要不删肉身"（合同见 `runtime-overview.md`「stage_compaction」）。台阶处 provider 前缀必然断一次：那一跳 `cache_hit_tokens` 塌回分叉点之前、非缓存 `input_tokens` 冲高，之后逐轮重新长回来——把这次断点读成缓存回归是错的。
