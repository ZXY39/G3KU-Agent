...
- `[G3KU_TOKEN_COMPACT_V2]` — 内联 LLM 全局压缩。第二行 JSON 为 `{"kind":...,"history_message_count":N}`：kind 为 `frontdoor_token_compaction_llm`（CEO/frontdoor）或 `node_token_compaction_llm`（节点，另带 `node_id` 与各分区计数字段），空行后接压缩摘要正文（语言跟随被压缩历史，不再强制中文）：模型直接继续阅读的自然语言会话内容，不是结构化数据，也不是 JSON。CEO/frontdoor 的正文尾部可能再带两节运行时管理的固定结构：`## 证据索引`（按模型选出的编号逐字回填的 `key_refs`，一 ref 一行）与 `## 阶段归档`（一行归档文件绝对路径 + stage 区间）；带归档时第二行 JSON 多一个 `stage_archive` 对象（`ref` / `stage_index_start` / `stage_index_end` / `stage_count` / `archived_through_created_at`；存量块里可能是 `stage_ids` 清单）。识别压缩块只看前缀与 `kind`，正文结构不是判定依据。
