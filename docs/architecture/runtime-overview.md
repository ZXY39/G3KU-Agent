...
- **节点正文在库内只存一份**：`task_node_details` 只有 `node_id / task_id / updated_at / payload_json` 四列，节点 input/output/check_result/final_output 等正文只落在 `payload_json` 的顶层平铺字段里；`TaskProjectionNodeDetailRecord.payload` 嵌套字典只允许放**没有平铺字段承载**的键（`parent_node_id / depth / node_kind / status / goal / execution_trace_summary / execution_trace / actual_request_* / token_usage* / direct_child_results / spawn_review_rounds / tool_file_changes`）。读侧取值式统一是 `payload.get(k) or record.<flat>`（`query_service.get_node_detail`），因此新旧两种形状都能读，缺键静默回退到平铺字段——这条优先级是"摘掉嵌套重复键不改行为"的前提，也是新增投影时的约束方向：先加平铺字段，再在 payload 里放同名键就是重复计数。`node_runner` 取子节点摘要是 `getattr(detail, 'output_text')` 直读平铺，不经 payload。护栏测试 `tests/resources/test_task_node_detail_single_copy.py`（钉住四列形状、正文在整行内只出现一次、payload 不得遮蔽平铺字段名、存量库迁移后旧行仍可读）。存量库首次打开由 `SQLiteTaskStore._setup` 逐个 `ALTER TABLE ... DROP COLUMN` 收掉平铺正文列，这是一次整表重写；删除失败（磁盘紧张、SQLite < 3.35）只记 WARNING 不阻断启动，写侧按 `PRAGMA table_info` 的实际列集取值，该表因此维持旧形状、重复照旧。库内另一份完整副本是 `nodes.payload_json`（整个 `NodeRecord`，含 `input/output/metadata`），它不在任何裁剪清单里，见「终态大行裁剪」条。
