...
- `waiting_children` recovery no longer outranks an active task-wide barrier. If a parent is blocked by `runtime_meta.distribution.blocked_node_ids`, the runtime must skip `waiting_children` replay until the barrier releases that node.
