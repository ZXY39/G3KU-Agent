...
任意一轮（用户轮、heartbeat、cron）的最终回复若为单独的 `[G3KU_SILENT]`（整串精确匹配，去除首尾空白），runtime 在 `_prompt_locked` 的 finalize 处把它识别为静默：不把可见回复投递到渠道（`message_end` 文本置空并带 `silent_reply` 标记）、不新增可见 assistant 转录条目（改落一条 `ui_visible=false` 的隐藏 assistant 以维持角色交替）、不回填请求体基线、不进 memory 复核。回合生命周期照常走完（`turn_end` / `agent_end` / `state_snapshot`、状态 `completed`），本回合中已发布的阶段与工具调用仍照常可见。`RunResult` 以 `output=""` + `is_silent_reply=True` 表达静默，channel transport 与 cron dispatch 的空输出守卫因此自然不投递。
...
- **删除渐进与墓碑（P3）**：压缩候选耗尽仍低于清理线时，按 `archived_at` 升序删除最老归档 zip（每批 2 个、批间水位复检、紧急线以下绝不执行、pinned 永不删）。墓碑 = `metadata.purged_at/purge_reason`（判定唯一入口 `TaskRecord.is_purged()`，**不进 TaskStatus Literal**——避免全链路状态校验与调度分支被波及）；tasks 行、节点结构、summary、error_logs、event-history gz 全保留，列表/详情对缺失文件天然容错。purged 任务 resume/decompress 被拒（`task_purged`），卡片显示「已清理」chip、只留 delete 动作。硬删仍只走用户 `delete_task` 链路。
- **终态大行 14 天裁剪（P3）**：`task_model_calls / task_runtime_frames / task_node_tool_results / task_node_rounds / task_node_details` 五张大行表按任务口径裁剪（终态且 `finished_at`（缺省 `updated_at`）早于 `detail_retention_days`，默认 14 天，0=关闭）。宿主并入每小时对账 loop（23h 间隔经 `maintenance_runs` 表跨进程卡权；紧急水位跳过——裁剪自身是写放大源）；每批 200 任务单事务 + `wal_checkpoint(PASSIVE)`。error_logs、tasks/nodes、task_events 行与 event-history 归档永久保留。
