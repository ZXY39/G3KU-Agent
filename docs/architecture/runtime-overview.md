...
- The runtime now treats appended task messages as a task-level control transaction: it persists a distribution epoch before ordinary execution sees the new message, and then routes the next-step delivery through either child mailbox rows or root-node pending notice metadata depending on the target.
...
- Root-node appended messages do not create `task_node_notifications` rows for the root itself. When the distribution epoch completes, the runtime writes node-local pending notice records onto the root node metadata; `_resume_react_state(...)` then closes the previous active stage, appends those root messages as user messages at the next safe boundary, and finally rolls them into the same durable `append_notice_context` used by child-mailbox deliveries.
