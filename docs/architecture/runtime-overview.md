...
- Task tree depth is bounded by two independent layers. The static layer is a creation-time clamp: `main_runtime.default_max_depth` (default 1) applies when `create_task` receives no `max_depth`, `main_runtime.hard_max_depth` (default 4) caps any requested value, and each node carries `can_spawn_children=(depth + 1) < task.max_depth` so the `spawn_child_nodes` tool is structurally gated — these are mechanical backstops, not model judgment. The dynamic layer is the spawn review: every new spawn batch (per call `call_id`; cached for retries, materialized rounds are never re-reviewed) is judged per candidate by the external inspection lane `spawn_child_review.md` via `review_spawn_candidates`. Review context carries `path_nodes` (root→parent path with each node's `goal` / `prompt` / `stage_goal`, full prompts being the task-boundary evidence), `parent_stages` (all completed stages plus the active stage of the spawning node, so the verdict aligns with stage progression), and `tree_summary` (compact outline of branches outside the path plus node count, so width explosion still registers). The verdict either admits candidates or returns per-candidate block reasons and next-step suggestions to the parent node. One review rule is a same-batch data-dependency gate: children of a single `spawn_child_nodes` call start concurrently with no ordering guarantee and the call returns only after every child pipeline is terminal, so a candidate that consumes its siblings' outputs (typically an acceptance/verification/aggregation node spawned together with the generators it inspects) is blocked with split-batch guidance — spawn the depended-on generators first, then the inspector in a later batch; per-child acceptance belongs to `requires_acceptance`, whose acceptance node activates only after that child's pipeline is terminal. A review-blocked child is returned to the parent as `SpawnChildResult.review_blocked=true` (check_result `派生已被拦截`) — a machine-readable "not created, never executed" marker the parent must act on via the accompanying reason/suggestion instead of counting the branch as completed; the blocked entry keeps `status=success` only as internal terminal bookkeeping and can never mask a live bound node (see the recovery invariant below). Spawn review usage merges into the parent node's `token_usage` via node-level accounting, so the task-level token aggregation includes review calls. There is no task-level depth capping lane.
...
- 对 CEO/frontdoor 主链路，每个请求只有一份最新契约：位于所有稳定前缀与持久化历史之后、当前 user 回合之前的一份 `frontdoor_runtime_tool_contract` system summary（运行时元数据、非对话内容）；模型输入末位保持当前 user 回合或合法的后续 assistant/tool 序列，避免契约块占据模型续写位置被回显。它属于“当前轮临时合同”，不是 durable history（剥掉/重注规则详见 `context-and-cache-troubleshooting.md`「append-only 规则」）。`normalize_model_output` 把 standalone 内部消息回显统一收口：运行时工具合同回显与阶段压缩块回显（`stage_prompt_compaction.py` 的三个 `[G3KU_STAGE_*]` 前缀，守卫按文本前缀自行判定）首次各导入一次私有修复提示——合同回显禁止引用合同，阶段块回显要求改用 `submit_next_stage` 或面向用户自然语言；重复回显转为不含内部材料的用户友好回退，阶段块原文绝不作为最终回复持久化或投递。可见答案尾部的契约/阶段块片段在 finalize 前剥除；渠道投递边界的 `sanitize_channel_outbound_text` 同样剥除两类回显作为最后防线。维护上区分 `dynamic_appendix_messages`（下一次重建时应追加的最新合同）与活动中的 `messages` / actual request JSON。
...
- 重试循环在每个退避边界对比 runtime config revision：revision 变化时抛出带 `config_revision_changed` 标记的可重试耗尽错误（`ModelProviderExhaustedError`），由上层重启回合/重建链，而不是继续用旧链空转。模型前进边界通过 `model_refs_resolver` 活解析模型链：运行中新加入的 fallback 模型在下一个模型边界可见，已试过的模型不回头重试。任务运行时侧见 `config-and-models.md`「模型链变更何时作用于在途回合」。
...
- **可降级写先过应急写预算**：actual-request artifact、`task.live.patch` 外置归档、execution trace 外置在写前调用 `has_emergency_disk_budget`（剩余空间 < max(`emergency_min_bytes`, 盘总量 × `emergency_min_ratio`) 即跳过全量落盘，退回 slim/minimal 形态）。预检带 5s TTL 缓存；探测失败保守放行。live patch 外置被跳过时 DB 行仍存 slim payload、`payload_archive_path` 为空（读端对空路径天然兼容）。
...
- **live patch flush 有界重试**：`flush_live_patch_history` 写失败时 entry 重新入队并重挂 2s timer，重试 3 次后丢弃并计数；`_append_task_event` 永不抛异常（返回是否落库），公开 `append_task_event` 失败返回 0。timer 线程与任务状态更新路径不会因磁盘满死亡。
...
- **终态即清中间产物**：任务迁移到 `success`/`failed` 时，终态监听器 `_cleanup_terminal_task_intermediates` 同步只做判定与 in-flight 去重，重活甩 daemon 后台线程：硬删 `temp/tasks/<id>` 草稿目录，并按保留清单删中间 artifact 的文件与 DB 行。保留清单（唯一权威）：`kind=='patch'`、`kind=='final_output'`、`task.final_output_ref` 指向的 artifact、标题含 `report`/`summary`；其余（`task_actual_request` / `task_runtime_messages` / `task_execution_trace` / `node_output` / `tool_result*` 等）全部删除。`task_error_logs` 表、节点 `blocking_reason`、`task_events` 行一律不动；`event-history/*.json.gz` 终态不即时删，按保留期批量清理（见「终态大行裁剪与事件归档保留期」）。用户删任务的 `delete_task` 链路是全删通道：任务临时目录按双路径兜底回收（runtime_meta 记录的实际路径优先，确定性默认路径兜底），硬删统一走 `fs_utils.remove_tree`——失败条目去只读位重试、Windows 下经扩展长度前缀删除超过 MAX_PATH 的深树（git 克隆的只读文件与超长路径不再造成静默残留），仍有残留时 loguru 告警，绝不静默。清理量记 loguru 日志并发 best-effort `task.intermediates.cleaned` 事件；读端对已删 artifact 由 `.exists()` / `read_artifact_text` 兜底，不炸。
...
- **任务归档与压缩渐进（P2）**：`main/storage/task_archive.py` 的 `TaskArchiver` 把任务四个磁盘目录（artifacts / event-history / files / temp）压成单个 zip（`.g3ku/main-runtime/task-archives/<safe_task_id>.zip`，内含清单 `.g3ku-archive.json`：uncompressed_bytes/file_count/per_dir/members）。提交点协议=先写完整 zip（tmp + CRC 校验 + 原子 replace）再逐文件删源——任意中断点满足"zip 有效即权威"，`recover_interrupted` 据此补删源或删无效 zip 回滚。解压预检 `uncompressed_bytes×1.2 < free`，不满足拒绝（任务保持原状）；解压经同盘 staging 目录逐文件校验大小后落位，成功才删 zip。排他双层：任务级 O_EXCL 文件锁（pid + 300s 过期回收）+ metadata `archived_at` 幂等判定。压缩前预检 `free > 紧急线 + 源大小×0.35`（zip 写入瞬时占位），不足返回 `insufficient_space` 且源完好。
- **压缩渐进调度**：清理线触发（monitor 边沿信号 `_disk_cleanup_requested` + embedded/worker 侧 60s 巡检 `_disk_cleanup_sweep_loop`）。候选 = 终态或已暂停任务、未归档、未 pinned、未 purged；排序 `score = size × max(1, 终态后分钟/1440)`（size×age 加权，大且老的先压）；每批 `archive_sweep_batch`（默认 4）个、批间 `archive_sweep_interval_seconds`（默认 15s）、每批复检水位、跌破紧急线立即停。压缩/解压在 embedded/worker 进程直接执行，web 模式经 worker 命令 `compress_task`/`decompress_task`/`start_archive_sweep` 转发（命令表在 `_process_worker_command`）。进度可观测：`worker_status_payload.disk_archive_sweep`（running/current_task/archived_count/purged_count/last_finished_at）→ 前端性能条「磁盘剩余」列追加「自动归档中（已压 N）」；单任务级进度即 `archived` 标记翻转（压缩对单任务是原子的）。解压进度走卡片「已压缩，正在解压…」hint（1s 轮询 archived 翻转，120s 上限）。
- **归档崩溃自愈**：压缩按"zip 校验通过（testzip+清单）才逐文件删源、最后才写 `archived_at`"的顺序推进，任何闪退窗口都有确定恢复语义，由清理线巡检每拍自愈——(a) 残留工作文件：`.tmp-*.zip`（压缩写一半）与 `.extract-*`（解压 staging）超过 10 分钟即清（`cleanup_stale_work_files`），在途文件不动；(b) zip 有效但源删到一半/元数据未落库：`_repair_interrupted_archives` 经 `recover_interrupted`（zip 有效即权威）补删残留源并从清单补写 `archived_at/archived_bytes`（reason=crash_recovery）、占用记账切到压缩后大小；(c) 损坏 zip 直接删除（源必然完好——删源只发生在 zip 校验之后）；(d) 反向窗口"有 `archived_at` 无 zip"（删除渐进后崩溃/手工删）：解压请求不静默清标记，转墓碑 `purged_at`（reason=archive_missing）。归档路径是确定性文件名，未落元数据的任务在下一轮压缩渐进会覆盖重写、天然自愈。解压中途磁盘被写满：`copyfileobj` 抛 OSError → staging 全清、返回 `insufficient_space`，zip 与源状态不变；解压幂等（无 zip 返回 True）。
- **归档任务的读端与恢复流转**：归档状态在 `TaskRecord.metadata`（`archived_at/archived_bytes/archive_compressed_bytes/archive_reason/pinned`，payload_json 零迁移），经 `TaskListItem.archived/pinned` 与 WS summary payload（含 fingerprint，翻转必推）到前端。`read_artifact_text` 在源文件缺失时自动回退读 zip 成员（`artifacts/<文件名>`，gz 成员两层解压；成员文本带 64MB 容量上限缓存）。resume 已压缩任务：`resume_task` 先 `_ensure_task_decompressed`（预检不足抛 `pending_decompress`，任务保持 paused）；前端卡片点恢复/点开归档任务显示「已压缩，正在解压…」（pause-hint 管线第三态，轮询列表 archived 翻转）。**解压宽限**：任何解压成功后写 `metadata.decompressed_at`，`decompress_grace_minutes`（默认 60，0=关闭）窗口内该任务不被压缩渐进重新归档——用户解压查看后不会立刻被系统压回去；窗口过后磁盘压力照常生效，手动压缩不受宽限限制。pin（书签）任务豁免压缩与删除渐进；`POST /api/tasks/{id}/pin` web 直写 DB 不经 worker。
- **删除渐进与墓碑（P3）**：压缩候选耗尽仍低于清理线时，按 `archived_at` 升序删除最老归档 zip（每批 2 个、批间水位复检、紧急线以下绝不执行、pinned 永不删）。墓碑 = `metadata.purged_at/purge_reason`（判定唯一入口 `TaskRecord.is_purged()`，**不进 TaskStatus Literal**——避免全链路状态校验与调度分支被波及）；tasks 行、节点结构、summary、error_logs、task_events 行全保留，列表/详情对缺失文件天然容错。purged 任务 resume/decompress 被拒（`task_purged`），卡片显示「已清理」chip、只留 delete 动作。硬删仍只走用户 `delete_task` 链路。
- **终态大行裁剪与事件归档保留期（P3/P3+）**：`task_model_calls / task_runtime_frames / task_node_tool_results / task_node_rounds / task_node_details` 五张大行表按任务口径裁剪（终态且 `finished_at`（缺省 `updated_at`）早于 `detail_retention_days`，默认 14 天，0=关闭）；每批 200 任务单事务 + `wal_checkpoint(PASSIVE)`。event-history 外置归档按 `event_history_retention_days`（默认 14 天，0=关闭）清理（`SQLiteTaskStore.prune_event_history_archives`）：超期终态且未 `metadata.pinned`、未 `archived_at` 的任务（pin 豁免审计回看，压缩归档豁免——解压回看需水合归档文件）删除外置归档目录并置空 `task_events` 行引用（`payload_archive_path=''`），行与 slim 预览保留，`list_task_events` 对缺失引用自动降级 slim；`delete_task` 全删链路不变。先删文件后清引用，中断由下一轮幂等补收；同轮顺带清扫 tasks 表已无对应行的孤儿归档目录（mtime 宽限窗保护未落库新任务，global 桶不扫）。两者宿主均并入每小时对账 loop（23h 间隔经 `maintenance_runs` 表跨进程卡权，key 分别为 `detail_prune` / `event_history_prune`；紧急水位跳过——清理自身是写放大/删除 IO 源）；事件归档清理后逐任务对账 `task_disk_usage` 实测值。error_logs、tasks/nodes、task_events 行永久保留。
...
维护者常见误读：把 `DiskFullError` 当新异常类型去 catch——它继承 `OSError`，既有 `except OSError` 分支自动覆盖；把终态清理当"数据丢失"——被删的只是中间产物，保留清单与 error_logs / task_events slim 预览保证可回顾性（event-history 外置归档超期清理后事件条目与预览仍在库）；在磁盘满排障时只看 `.g3ku/errors/`——磁盘满期间错误日志本身可能是 0 字节空文件，权威信号是 `write_failure_counts` 与 worker 日志里的 SQLITE_FULL 行；把紧急态下"工具不动了"当卡死——那是 target_limit=0 的排队等待，任务随即被自动暂停，恢复磁盘空间后手动 resume。
...
- In web mode, node-control actions dispatch to the task worker as one `task_commands` entry per action: `pause`→`pause_node`, `resume`→`resume_node`, `fail`→`fail_node`. `keep_paused` is leader-local only — it re-registers the pause row and remark and enqueues no worker command (the worker has no `keep_paused` command type, and an enqueued command with that action would otherwise be applied as a fail). A `fail_node` command carries the operator/agent remark in its payload, and the worker falls back to that remark as the failure reason, so a genuinely failed node records the actual diagnosis instead of the generic default text.
