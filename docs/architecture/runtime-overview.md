...
- 裁撤由模型在关闭阶段时点名：`submit_next_stage` 的 `drop_completed_stage_tool_detail` 为真时，正在关闭的那条阶段落 `context_evicted: true`（前门写 dict 账本、节点写 `ExecutionStageRecord`，两条车道共用同一个 `retained_completed_stage_ids` 判据，该判据另有 `raw_stage_renderer.retained_completed_raw_stage_ids` 一份本地镜像，改动必须同步）。它要求同批带非空 `completed_stage_summary`——库里 46.2% 的终态阶段总结本就是空的，没有总结就裁等于把该阶段唯一的记录一起移走。未点名的阶段仍走窗口 3 的默认规则。移出只作用于 provider 可见层：账本 `rounds` 与 `task_node_tool_results` 一条不减。节点侧回读走 `task_node_detail`（对 agent 固定返回完整执行轨迹，按 `stage.rounds[].tool_call_ids` 关联逐条 `arguments_text` / `output_ref`）；前门侧走**裁撤导档**：`_frontdoor_archive_evicted_stage` 在被裁撤那条阶段的账本落到本轮副本之后，把它的记录（`kind=frontdoor_stage_eviction`）写进 `session_temp_dir`，路径落在账本 `archive_ref` 上、由块逐轮带出，模型用 `content_open` 打开。**前门不做专用读取工具**：送给 provider 的工具 schema 由 `_selected_tool_schemas` 从**全局** `loop.tools` 注册表按名字解析（`bootstrap_bridge` 里那句「光靠 callable 名单点名不够，工具必须先存在于 registry」就是这条），而阶段账本是**每会话**的——把带闭包的实例注册进全局表等于把 A 会话的账本暴露给 B 会话，所以绕开工具面复用 `content_open`。一条阶段只导一次（ref 已存在即复用）；导档失败就不写 ref、块里也不出现指针。两条车道的块都带 `evicted` 字段标明"这条是你自己移走的"，没有它模型不知道该不该回读、事后也无法从发送体核对裁撤被用过没有。有一批帧任何窗口都裁不到（`submit_final_result` 被拒轮与单独提交的 `submit_next_stage` 边界轮不进 `rounds`），读正文尺寸前先按「Shrink 原因与压缩边界」的不可回收地板扣掉它。该标记穿过前门四处重写者（`_normalize_stage` 白名单、`stage_state` 快照白名单、`_dedupe_canonical_stages` 的跨副本继承、`_completed_stage_overlap_signature` 的剔除）——收口标记曾在同一批落点上漏过一次导致块整批长回来，裁撤标记走的是同一批。
