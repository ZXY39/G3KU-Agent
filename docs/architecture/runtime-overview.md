...
静默由一个**工具调用**表达，而不是由回复文本来表达。模型在任意一轮（用户轮、heartbeat、cron）调用 `silent`，本轮就不向用户投递任何回复；参数 `reason`（必填）/ `subject` / `superseded_by` 同时是审计载荷和痕迹内容。这个工具是前门专属的常驻内置控制工具，曝光与终态语义详见 `tool-and-skill-system.md`「常驻内置控制工具」。
...
- ReAct protocol-repair circuit breakers (invalid final/stage submissions, mid-stage plain-text replies, repeated read-only calls, stage-only loops, XML pseudo-tool repair exhaustion, orphan tool-result detection, and standalone runtime-tool-contract echo) return an internal `failure_disposition=pause`. A plain-text answer while a stage with unused budget is active is re-prompted with a stage-protocol repair overlay instead of being submitted on the node's behalf; the lane split and its limits are owned by `tool-and-skill-system.md`「阶段门控与 callable 收紧」. Every lane that rejects a `submit_final_result` shares one strike budget and one strike handler: a submission rejected by the tool's parameter contract, by the result-contract validator, for not being alone in its turn, or its absence, counts one strike and is re-prompted with a repair overlay, converting into the error pause on the last strike (`_INVALID_FINAL_SUBMISSION_LIMIT`) or when the ReAct iteration budget expires on a still-rejected submission — that second exit returns the same guard failure instead of raising, because an escaping exception is only caught by `NodeRunner`'s generic handler, which checks neither explicit-pause/cancel precedence nor the node's last valid result flush. The counter lives on the node frame so it survives restarts, resets on any ordinary tool turn (together with the recorded contract-violation list, so an expired strike can never be re-cited at loop exit), and each strike writes its own `task_error_logs` record with the truncation forensics — a single schema slip is never terminal, and no breaker is preempted by another one firing early (an orphan tool-result history is reported by the orphan breaker). A standalone contract echo is repaired exactly once by re-prompting the model with a private repair note; a repeated echo is converted into this error pause instead of being auto-wrapped into a `success + final` node result. `NodeRunner` keeps that field out of the persisted result payload, records the guard reason in `task_error_logs`, and registers an `error` pause while the node stays `in_progress`; normal `submit_final_result(status="failed", ...)` business outcomes remain terminal failures. Explicit pause, cancellation, and task-terminal state take precedence over this conversion. Orphan tool-result detection additionally warns each strike into the worker log with the task, node, and orphan call ids, so this pause class is diagnosable from the worker log without DB forensics. Runtime self-fault is a separate breaker class beside those protocol lanes: a tool executor raising an engine-level defect (`NameError` / `UnboundLocalError` / `AttributeError` / `ImportError` / `TypeError` whose deepest traceback frame lands inside `main/` or `g3ku/`) counts per `(node, tool, exception text)` and, after `_TOOL_FAULT_LIMIT` identical hits, converts into the same error pause with guard reason `runtime_fault:<tool>:<text>`. The key is a signature rather than consecutive turns because the real failure shape interleaves identical faults with other tools' successful turns; a tool's entry is dropped the moment that same tool returns normally. Ordinary tool errors never count — parameter validation, missing paths, and provider messages stay in-turn feedback for the model.
...
- A child pause leaves its dispatcher future pending so the parent pipeline waits without treating the child as a failed result. Every run of a spawned child — including the rework run after an acceptance rejection — goes through that dispatcher entry, so a pause at any point of the child's life leaves the parent waiting inside the same `spawn_child_nodes` call instead of receiving a child failure. The root pause propagates to `TaskActorService`, which leaves the task runnable state paused. Failing a paused child resolves the original future with a failed result so the parent can apply its ordinary failure handling. Freeze signals (`NodePausedError` beside `TaskPausedError` and `DistributionHoldError`) are never converted into a spawn entry outcome at any nesting level: a settlement lane that swallows one hands the parent model a child failure it cannot retract, and the parent's next round sweeps the still-running subtree as `superseded by newer spawn round`.
...
- Resume clears the node pause state and must leave a live executor; clearing flags is not resuming. A resume aimed at a node whose spawn entry already settled as failed inside a round the parent has been handed (`spawn_operations[round].completed`) is refused with `entry_settled` before any flag is cleared, and no worker command is enqueued for it — reviving that node produces work no waiter will adopt, and its parent's next round sweeps it together with its bound acceptance node. The worker replays accepted `resume_node` commands with `force=True` and does not re-veto, which is sound precisely because a refused node never reaches the queue. The `resume_node` command lane and the epoch-release lane share one dispatch-entry triage: a pending entry future is re-run in place, an entry whose future already resolved while its coroutine stopped is discarded and rebuilt (a rebuilt future left in place would receive a result nobody awaits), and a resolved future whose coroutine is still running is deliberately not rebuilt so a node cannot run twice. A resume for a node with no entry creates that entry. The triage outcome is logged per node with task and node ids, a resolved-but-unconsumed future logs its stranded exception, and the node enters the delayed verification sweep that re-resumes it once when it is still wedged. The invariant this protects: a non-terminal node is never left with neither an executor nor a scheduled re-resume — the `run_task`-pickup orphan reconciliation is the last resort that makes the leftover visible, not the mechanism that recovers it. `NodeRunner._resume_react_state` restores the persisted `task_runtime_frames` frame, allowing the node to continue from its prior runtime context instead of starting a fresh node turn. Distribution barriers treat paused nodes as part of the in-progress frontier and do not wait on a paused child as if it were an unresolved safe-boundary drain.
...
- 原位放置是缓存硬约束：压缩块不整体收拢到上下文头部；除治愈遗留布局的一次性收敛外，每次压缩的前缀失效面从最早被压缩阶段的位置开始。断点**位置**不随保留窗口移动（它永远钉在"最老的仍带肉身阶段"的首帧，节点上实测在 index 19，之前 19 帧 = system + 引导 user + 已裁阶段的块 ≈ 3.7 万字符 ≈ 1.25 万 tok，与实测裁后 `cache_hit` 1.2–1.7 万 tok 同量级），随窗口移动的是**断点后面还要重传多少**：窗口 3 时那里压着三个完整阶段（实测一条活节点 27.4 万字符 ≈ 9.2 万 tok，那一跳非缓存 `input` 从正常的 1.5–3 千冲到 7.4 万）；一关阶段就裁时断点后面只剩刚开的 0–2 轮（实测每跳增量 3 帧 ≈ 4.3 千字符 ≈ 1.4 千 tok）。所以"攒批再裁"是反向杠杆（实测按 3 批一次约多付 25 倍），要省只能更早裁；而断点次数只由阶段数决定，与窗口无关。
- 块是"按账本逐轮重渲染"的产物，不是历史消息：每轮从 `frontdoor_stage_state` / `frontdoor_canonical_context` 的合并视图重新生成一份块集合，因此仅重写请求体基线（`token_compression`）不会让块变少——收口标记才是。不占 raw 保留窗口的阶段有两类：带 `context_visible: false` 的已收口阶段，与带 `context_evicted: true` 的已裁撤阶段。两者信息损失边界不同，不可混用：收口连块一起不再渲染（正文已进全局摘要），裁撤只移出工具肉身、块照旧逐轮在场（块里那条总结就是它承诺的唯一留存记录）。已收口阶段若其工具肉身被别的路径重投影回体内，按原位移除且不再补块，这是收口的既定信息损失边界。
- 裁撤由模型在关闭阶段时点名：`submit_next_stage` 的 `drop_completed_stage_tool_detail` 为真时，正在关闭的那条阶段落 `context_evicted: true`（前门写 dict 账本、节点写 `ExecutionStageRecord`，两条车道共用同一个 `retained_completed_stage_ids` 判据，该判据另有 `raw_stage_renderer.retained_completed_raw_stage_ids` 一份本地镜像，改动必须同步）。它要求同批带非空 `completed_stage_summary`——库里 46.2% 的终态阶段总结本就是空的，没有总结就裁等于把该阶段唯一的记录一起移走。未点名的阶段仍走窗口 3 的默认规则。移出只作用于 provider 可见层：账本 `rounds` 与 `task_node_tool_results` 一条不减。节点侧回读走 `task_node_detail`（对 agent 固定返回完整执行轨迹，按 `stage.rounds[].tool_call_ids` 关联逐条 `arguments_text` / `output_ref`）；前门侧走**裁撤导档**：`_frontdoor_archive_evicted_stage` 在 finalize 重建 durable 账本那一步（`_frontdoor_stage_state_after_tool_cycle` 重放 `submit_next_stage` 之后）给被裁撤的阶段写一份档（`kind=frontdoor_stage_eviction`）到 `session_temp_dir`，路径落在 durable 账本 `archive_ref` 上、由块逐轮带出，模型用 `content_open` 打开。导档点不放 `submit_next_stage` 的图闭包：闭包改的是本轮工作副本，durable 账本随后被重放返回值整个覆盖，写在副本上的 ref 等于没写（文件照落盘，块里却永远只有 `evicted` 没有指针）。**前门不做专用读取工具**：送给 provider 的工具 schema 由 `_selected_tool_schemas` 从**全局** `loop.tools` 注册表按名字解析（`bootstrap_bridge` 里那句「光靠 callable 名单点名不够，工具必须先存在于 registry」就是这条），而阶段账本是**每会话**的——把带闭包的实例注册进全局表等于把 A 会话的账本暴露给 B 会话，所以绕开工具面复用 `content_open`。一条阶段只导一次（ref 已存在即复用）；导档失败就不写 ref、块里也不出现指针。两条车道的块都带 `evicted` 字段标明"这条是你自己移走的"，没有它模型不知道该不该回读、事后也无法从发送体核对裁撤被用过没有。有一批帧任何窗口都裁不到（`submit_final_result` 被拒轮与单独提交的 `submit_next_stage` 边界轮不进 `rounds`），读正文尺寸前先按「Shrink 原因与压缩边界」的不可回收地板扣掉它。该标记穿过前门四处重写者（`_normalize_stage` 白名单、`stage_state` 快照白名单、`_dedupe_canonical_stages` 的跨副本继承、`_completed_stage_overlap_signature` 的剔除）——收口标记曾在同一批落点上漏过一次导致块整批长回来，裁撤标记走的是同一批。
- 该规则与是否存在活动阶段无关；无活动阶段的纯对话回合同样压缩过期阶段并保留最近 3 个（窗口只服务未被点名裁撤的阶段）。
