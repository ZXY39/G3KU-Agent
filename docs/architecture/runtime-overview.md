...
- **全删渐进（清理线触发）**：清理线信号（monitor 边沿 `_disk_cleanup_requested` + embedded/worker 侧 60s 巡检 `_disk_cleanup_sweep_loop`）触发 `_wipe_progressive_sweep`：候选 = 终态（success/failed）且 `finished_at` 早于全删宽限窗（`_FULL_DELETE_GRACE_HOURS`，24h 模块常量——刚跑完的任务不被立即不可逆删除）且不在台账内的任务；先取最老 50 个，按 `size × max(1, 终态天数)` 加权排序（size = 五张大行表 payload 字节 `sum_task_detail_bytes` + `task_disk_usage` 目录记账），大且老的先删；每批 2 个、每批复检水位、紧急线以下绝不执行；删除前产出照常导出。`purge_enabled=False` 停用整档。
- **终态大行裁剪（P3）**：`task_model_calls / task_runtime_frames / task_node_tool_results / task_node_rounds / task_node_details` 五张大行表按任务口径裁剪（终态且 `finished_at`（缺省 `updated_at`）早于 `detail_retention_days`，默认 7 天，0=关闭）；每批 200 任务单事务 + `wal_checkpoint(PASSIVE)`。宿主并入每小时对账 loop（23h 间隔经 `maintenance_runs` 表跨进程卡权，key `detail_prune`；紧急水位跳过——裁剪自身是写放大源）。error_logs、tasks/nodes 结构、task_events 行保留至任务删除（wipe 全删）。
...
维护者常见误读：把 `DiskFullError` 当新异常类型去 catch——它继承 `OSError`，既有 `except OSError` 分支自动覆盖；把终态清理当"数据丢失"——被删的只是中间产物，保留清单与 error_logs / task_events 行保证可回顾性（产出在删除任务时自动导出 deliverables）；在磁盘满排障时只看 `.g3ku/errors/`——磁盘满期间错误日志本身可能是 0 字节空文件，权威信号是 `write_failure_counts` 与 worker 日志里的 SQLITE_FULL 行；把紧急态下"工具不动了"当卡死——那是 target_limit=0 的排队等待，任务随即被自动暂停，恢复磁盘空间后手动 resume。
