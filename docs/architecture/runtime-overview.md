...
- `RuntimeAgentSession` 把自主复核窗口缓冲在 `memory/review_state.json`，并在两个时点自动入队直接 `write` 批次：配置的普通轮窗口阈值、压缩阶段冲刷（`token_compression` / `stage_compaction`）。不支持的冲刷来源必须保持缓冲窗口原样，不得制造队列行。复核载荷与用户在界面看到的可见面一致：用户轮记录用户消息与助手回复；心跳/cron 内部轮只记录可见的助手回复与阶段可见面，隐藏事件束提示词不进载荷；静默 `HEARTBEAT_OK` 内部轮不入队。每轮载荷含「可见工具记录」段，收集阶段可见面中仍保留 `rounds` 的阶段（近 3 个完成阶段的 RAW 保留窗口）里新出现的工具调用：工具名、状态、入参提示（写入侧已限长）、输出预览（截 240 字）、输出全文（截 400 字）与不截断的伴随中途输出（round.text，同一 round 只附一次）；每轮上限 8 条、错误优先、最新优先。工具记录按会话已上报 cursor（上限 500；空 `tool_call_id` 用内容指纹）跨轮去重。`review_state.json` 除 `pending_turns` 外维护阶段 delta cursor 与工具记录 cursor：冲刷只清缓冲轮次、保留两个 cursor，只有显式清除会话复核状态才连同丢弃。`stage_summary` 基于 stage delta：只含相对上一次复核新出现或实质变化的阶段。
