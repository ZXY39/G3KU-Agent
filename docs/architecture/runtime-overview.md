...
- Recovery replays no longer outrank task-wide append-notice priority. If a node is blocked by `runtime_meta.distribution.blocked_node_ids`, the runtime must skip interrupted-turn recovery such as `pending_tool_turn` or `waiting_children` replay until the barrier releases that node. After the epoch reaches `resume_ready`, most nodes still listed in `pending_notice_node_ids` must also skip those recovery replays until their local pending notice or mailbox delivery has been consumed into ordinary execution.
...
- `resume_ready` means distribution decisions are finished but some node in the live tree still has an unconsumed local notice or mailbox delivery. This is now a recovery-first phase, not a free fall back to ordinary root execution: the runtime dispatches `pending_notice_node_ids` first, lets `wait_for_children` nodes replay their existing child round without opening a new one, and only after that recovery pass does the held notice become eligible for ordinary prompt consumption. UI/ops should keep treating the task as being inside the append-notice transaction until `pending_notice_node_ids` is empty. After each notice-consumption or recovery-only pass, the runtime revalidates this list against durable pending root notices and delivered mailbox rows; if no real pending message remains, it clears stale `pending_notice_state` markers and empties `runtime_meta.distribution` so the frontend can leave yellow distribution/pending-notice mode.
...
- `memory/review_state.json` therefore now keeps a per-session stage cursor in addition to `pending_turns`. Flushing a window clears only the buffered turns; it must preserve that stage cursor so later windows do not replay already-reviewed stages back into a new `assess` payload. Only an explicit session review-state clear should drop both the pending turns and the remembered stage versions.
...
- The final preflight estimate no longer treats inline image `data:` URLs as ordinary text whose token cost scales with base64 length. Frontdoor now strips inline image bytes out of the estimation payload, estimates text/schema cost separately, and adds a dedicated image-token heuristic instead.
