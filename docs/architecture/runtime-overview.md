...
- 阶段收口是 CEO/frontdoor 压缩的第三件事：本次真正被吞掉的完成普通阶段（= 完成普通阶段 − 最近 3 个 raw 保留窗口 − 活动阶段 − 肉身或旧块仍留在保留尾部里的阶段）被登记成一份清单，连同完整记录（逐条 `key_refs` 与轮次）确定性导出到 `session_temp_dir` 下的归档文件。清单挂在压缩块的 `stage_archive.stage_ids` 上，**标记本身在 durable 基线推进的那一步（`_persist_frontdoor_actual_request`）才被应用**——压缩算完不等于 provider 看到了摘要，若那次发送随后失败或被暂停，基线仍是带块的旧请求体，此时翻标记就等于把那批阶段连同摘要一起丢掉。归档文件写不出来时整轮不收口（没有 `stage_archive` 就没有清单）：宁可不缩，也不能把阶段收进一个模型打不开的地方。不删账本、不改内容、不动 `stage_index` 序列：阶段只是不再进 provider 上下文，账本仍是 Web 时间线的权威。标记必须跟着**逻辑阶段**走而不是跟着 `stage_id` 走：`frontdoor_canonical_context` 与 `frontdoor_stage_state` 各自维护一套 stage_id 序号（同一会话实测 1..382 对 843..1226、交集为 0），而合并视图去重后存活的是较新的 `frontdoor_stage_state` 副本——只按 id 标记等于完全没收口，块会整批长回来。因此写标记时按内容身份（`created_at` + `finished_at` + `stage_goal` + `completed_stage_summary`，与跨 rebase 去重同一口径）在两份存储里一起打，两级去重（按 id、按内容身份）也把任一副本上的标记转移到存活副本。收口只能随压缩边界发生——逐轮按"块太多"自行收口会破坏 append-only。
