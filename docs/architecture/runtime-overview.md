...
- 原位放置是缓存硬约束：压缩块不整体收拢到上下文头部；除治愈遗留布局的一次性收敛外，每次压缩的前缀失效面从最早被压缩阶段的位置开始。节点上这条要按实测读：被裁的永远是**最老**的终态阶段，它就排在 bootstrap 之后第 3 条左右，所以"从最早被压缩阶段起断"在节点正文里等于近乎整体重传（实测一块之前的前缀只占正文 13.5%，裁后 provider `cache_hit` 塌到 1.2–1.5 万 tok）。这笔重传是划算的：一次性多付 ~6–7.5 万非缓存 tok，换裁后每跳少 1.7–3.3 万 tok，2–4 跳回本，而一条腿通常 18–28 跳。**反向的"攒批再裁"是错的杠杆**：多带 N-1 个过期阶段的肉身按跳累加，远大于省下的那几次断点（实测按 3 批一次约多付 25 倍），要减少代价只能更早裁，不能更晚裁。
