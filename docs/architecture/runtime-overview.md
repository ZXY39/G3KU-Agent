...
- **归档任务的读端与恢复流转**：归档状态在 `TaskRecord.metadata`（`archived_at/archived_bytes/archive_compressed_bytes/archive_reason/pinned`，payload_json 零迁移），经 `TaskListItem.archived/pinned` 与 WS summary payload（含 fingerprint，翻转必推）到前端。`read_artifact_text` 在源文件缺失时自动回退读 zip 成员（`artifacts/<文件名>`，gz 成员两层解压；成员文本带 64MB 容量上限缓存）。resume 已压缩任务：`resume_task` 先 `_ensure_task_decompressed`（预检不足抛 `pending_decompress`，任务保持 paused）；前端卡片点恢复/点开归档任务显示「已压缩，正在解压…」（pause-hint 管线第三态，轮询列表 archived 翻转）。pin（书签）任务豁免压缩与删除渐进；`POST /api/tasks/{id}/pin` web 直写 DB 不经 worker。
