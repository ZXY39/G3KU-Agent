...
- **投影正文在库内只存一份**：`task_node_details` 只留 `node_id / task_id / updated_at / payload_json`，`task_node_tool_results` 只留三个主键列 + `order_index`（被索引）+ `payload_json`——其余平铺列（input/output/arguments_text/output_preview_text 及一堆小列）在建表后就不存在，因为四个读点一律 `SELECT payload_json`，平铺列零读者。规则：列只承担主键与索引，正文一律只落 `payload_json` 顶层；`payload` 嵌套字典只允许放**没有平铺字段承载**的键（明细表的 `goal / parent_node_id / depth / node_kind / status / execution_trace_summary / execution_trace / actual_request_* / token_usage* / direct_child_results / spawn_review_rounds / tool_file_changes`，工具结果表的 `parsed_payload`）。读侧取值式统一是 `payload.get(k) or record.<flat>`（`query_service.get_node_detail`），新旧两种形状都能读、缺键静默回退——这条优先级是"摘掉重复键不改行为"的前提，也是新增投影时的约束方向：先加平铺字段、再在 payload 里放同名键就是重复计数。`node_runner` 取子节点摘要是 `getattr(detail, 'output_text')` 直读平铺，不经 payload。存量库首次打开由 `SQLiteTaskStore._drop_legacy_columns` 做**一次**表重建收掉旧列：新表定义从 `PRAGMA table_info` 反向合成（含合成 `PRIMARY KEY`，否则工具结果表的复合 `ON CONFLICT` 失效），`INSERT..SELECT` 拷数据后改名，索引段在其后重建。绝不允许改回逐列 `ALTER TABLE DROP COLUMN`——SQLite 每删一列整表重写一次，实测 11 列把启动阻塞近 3 分钟（端口无人监听）、WAL 冲到 2.7 GB。重建失败只记 WARNING 不阻断启动，写侧按实际列集取值，该表维持旧形状。护栏测试 `tests/resources/test_task_node_detail_single_copy.py`（两张表形状、正文整行内只出现一次、payload 不得遮蔽平铺字段名、复合键 upsert 就地更新、存量库迁移后旧行可读且索引回来、一次重建而非 N 次 DROP）。库内最后一份完整副本是 `nodes.payload_json`（整个 `NodeRecord`，含 `input/output/metadata`），不在任何裁剪清单里，见「终态大行裁剪」条。
