...
- Recovery replays no longer outrank task-wide append-notice priority. If a node is blocked by `runtime_meta.distribution.blocked_node_ids`, the runtime must skip interrupted-turn recovery such as `pending_tool_turn` or `waiting_children` replay until the barrier releases that node. After the epoch reaches `resume_ready`, any node still listed in `pending_notice_node_ids` must also skip those recovery replays until its local pending notice or mailbox delivery has been consumed into ordinary execution.
