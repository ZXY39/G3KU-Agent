...
- That sidecar is now a parent-facing recipient projection rather than the raw live-child set. Execution nodes in `acceptance_handshake.state="waiting_acceptance"` are intentionally hidden from parent-facing child lists, while unfinished acceptance nodes remain visible and may still receive distribution messages.
...
- First ordinary consumption of a held append-notice must preserve the previous provider-facing request as an exact prefix. When a latest actual-request artifact has `provider_request_body.input`, `_resume_react_state(...)` uses that provider input as the resume seed and appends the new notice after it, rather than rebuilding from projected `request_messages` or `runtime_frame.messages`. This keeps the old context fully cacheable; only the newly appended notice and live tail should be uncached.
...
- Child acceptance and root `final_acceptance` now share the same reflation loop. The first rejection feeds the acceptance feedback back into the execution node, preserves the acceptance node context, and reactivates execution for one retry. The second rejection is terminal; execution failure during the retry cancels the waiting acceptance path instead of routing through acceptance again.
