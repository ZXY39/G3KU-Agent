...
- A silent reply is a live-only exception with two forms, neither of which reaches the user. `HEARTBEAT_OK` is the internal-turn ACK: an internal turn's real assistant reply is durable transcript history and, whenever non-empty and not `HEARTBEAT_OK`, finalize folds it into the request-body baseline (keyed on output content, never on turn kind). `[G3KU_SILENT]` is the universal silent-reply sentinel (see `runtime-overview.md`「静默回复」): any turn may end with that exact token and the runtime swallows the reply from the channel and the baseline — nothing is delivered or folded back, and the transcript instead persists a visible `信息已静默` placeholder for the Web frontend — while the turn lifecycle and any stage/tool activity still complete normally. Baseline forensics: `context-and-cache-troubleshooting.md`「finalize 没把 direct reply 补回 baseline」.
...
- The runtime does not hard-code a prompt-side ban on cron-tool mutations during reminder turns: if the reminded work itself is a currently visible CEO action (e.g. sending a reminder, creating another cron), the model may use the ordinary CEO tool surface — with `cron(remove)` limited to the firing job itself (合同见 `tool-and-skill-system.md`「`cron` 工具合同」).
...
Cron job delivery is claim-before-dispatch with a bounded, self-finalizing dispatch — the restart/recovery guarantee and the live-process hang guarantee are one contract:
...
- Every dispatch runs as an independent asyncio task and is finalized on **every** exit path, so the store never stays in `running` while the process lives (`timeout`/`interrupted` are the live-process equivalents of the startup `interrupted` reconciliation):
  - handler returns → `ok`, `delivered_runs` advances, terminal `max_runs` removal applies;
  - handler raises → `error` (one-shot `at` retries after 60s, recurring recomputes its schedule);
  - delivery watchdog expires (`cron.dispatchTimeoutSeconds`, default 1800s, `<=0` disables; see `config-and-models.md`「`cron`」) → the dispatch task's live await chain is dumped to the log (primary post-mortem for a wedged `session.prompt`), the task is cancelled, and the finalize waits a bounded grace (`cron.dispatchCancelGraceSeconds`) for the unwind → `timeout`. A task that resists cancellation is abandoned detached (loud error) and the job **stays in-flight** until the task really ends, so later ticks log `still in flight; skipping this tick` instead of stacking a second dispatch on the wedged session;
  - cancellation (service `stop()`, shutdown) → `interrupted`; the inner handler task is cancelled too.
  Unfinished one-shot `at` dispatches (`timeout`/`interrupted`) follow the same at-most-once suppression as the restart case: `next_run` cleared and the job disabled — the dispatch may already have reached the downstream handler, and a duplicate reminder is worse than a missed one.
- One wedged dispatch can never pin the scheduler: the timer tick claims due jobs synchronously, spawns their dispatch tasks, and always re-arms in a `finally`. In-flight jobs are excluded from wake arithmetic (their stale past `next_run` would otherwise spin a zero-delay tick loop); each finalize re-arms the timer to pick up its job's freshly advanced schedule. A tick task that dies unexpectedly logs `Cron: timer task died unexpectedly` and self-heals with a delayed re-arm — the scheduler never goes silent without a greppable trace.
...
- Hang forensics chain (three complementary instruments): the bridge slow-prompt watchdog warns with the live await chain when any `SessionRuntimeBridge.prompt` exceeds `G3KU_SLOW_PROMPT_WATCHDOG_SECONDS` (default 900s); the cron dispatch watchdog dumps the chain again at its own timeout and then kills the dispatch; the session finalize-tail profiler (`turn finalize tail slow ...` warning) attributes slow-but-completed tails to a stage (transcript persistence / memory-review bookkeeping / terminal emits).
...
- External-bridge reply delivery: for an `ext:` session, the persisted reply is also handed to `reply_notifier` → `_notify_heartbeat_channel_reply`, which publishes an `OutboundMessage(channel="ext")`; the shared drain resolves it via the external session registry and emits `outbound.created` on the session's event hub. Legacy `china:*` sessions lost their delivery path with the China channel subsystem removal and are skipped silently (transcripts stay readable archives). Session-meta derivation must never use a naive first-colon split — see `_derive_session_channel_chat`. Reply persistence failure (e.g. disk-full during transcript save) never skips the notify: the reply stays on the in-memory session (written out by the next successful save) and both `ceo.reply.final` and the channel notifier still run — a failed save must not silence the only user-facing surface. The wake loop (`SessionHeartbeatWakeQueue._run`) tolerates leaked handler exceptions the same way: log, then retry on a bounded backoff instead of letting the task die and strand every queued event for that session. Contract details: `external-agent-api.md`「出站路由」; delivery durability across pump outages and restarts: `external-agent-api.md`「持久 outbox」.
...
- After 5 consecutive failures the node enters escalation: events stay queued and keep retrying at the 5-minute cap, a visible "these nodes failed 5 times — fail them to let the task continue?" reply is emitted once when the cap is crossed (this is the model-unavailable fallback, since the escalation directive injected into later deliveries only reaches the user once the model can respond), and the node stays paused for the user or a later turn to decide. The escalation reply is delivered even when its own transcript persistence fails (disk pressure): notify is memory-first and the failed save only logs — otherwise the model-unavailable fallback goes silent exactly during the disk-full storms that need it most.
...
- Prompt contract: `shutdown_resume` joins `task_terminal` in the must-not-be-`HEARTBEAT_OK` family. The model must continue the paused request and end with a user-visible reply; a silent `HEARTBEAT_OK` / empty output enters the same repair loop as task-terminal, and exhaustion produces a fixed visible fallback text. The sole silence escape hatch for this family is `[G3KU_SILENT]`: it is recognized before the repair loop and closes the turn with no user-visible reply (acked, not delivered).
...
- Inline executions register in `InlineToolExecutionRegistry` (not the detached `ToolExecutionManager` background-execution path). Patrol scheduling is self-adaptive, not fixed windows: the first check fires at `DEFAULT_FIRST_CHECK_SECONDS` (120s; before that the sidecar is entirely silent), and every `CONTINUE` decision may carry the next check interval (`CONTINUE <seconds>`, clamped to `[30, 600]`). Deterministic-skip rounds and unusable-decision rounds inherit the previous round's interval, so they keep the cadence without a model call.
- The sidecar patrols within the universal tool timeout deadline but can never extend it: the deadline is the mechanical backstop, the sidecar is an early-stop optimization. A self-scheduled check landing after the deadline is harmless — the outer layer already terminated the execution and the patrol loop exits on the terminal record state. The universal timeout contract itself (default value, argument override, self-enforcing vs outer enforcement split, timeout error text) is owned by `tool-and-skill-system.md`「统一工具 Timeout 合同」.
- With an authoritative CEO actual-request JSON, `CeoToolReminderService` reuses that saved `request_messages` / `tool_schemas` / `prompt_cache_key` / `parallel_tool_calls` scaffold as cache prefix and appends only live reminder-tail messages (the stats line and the decision instruction); otherwise it falls back to a read-only `CeoMessageBuilder.build_for_ceo(..., ephemeral_tail_messages=...)` rebuild.
- The sidecar reuses the CEO main model binding; its decision channel is a single line — `STOP`, or `CONTINUE <seconds>` (JSON `{decision, next_check_seconds}` also accepted) — and even when it reuses the main turn's full provider-visible tool bundle for cache-prefix stability, it must not execute arbitrary returned tool calls.
- The reminder decision prompt carries the judgment statistics: total elapsed runtime, the call's maximum runtime when known, total reminder count, keep-waiting counts split by source (model decisions vs automatic observation skips), and seconds elapsed since the last keep-waiting decision. The decision is still observation-aware before the model is consulted: the sidecar reads the current tool name, normalized arguments, and the latest live `sidecar_observation` payload when available. `sidecar_observation` is a generic progress-side channel; `agent_browser` is the first producer. See `web-and-admin.md`「CEO Live Tool Reminder Contract」 for UI rendering of reminder events.
...
- If the reminder model call fails, times out, or returns unusable output, the default decision is `unavailable`; `unavailable` reminders do not stop the tool, do not interrupt the main turn, and inherit the previous round's interval for the next check.
- Only an explicit sidecar stop decision is allowed to stop the inline execution. A universal-timeout hard stop is a separate mechanical path, not a sidecar decision.
