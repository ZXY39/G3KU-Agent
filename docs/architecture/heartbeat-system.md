...
- One-shot `at` creation is also guarded against duplicate registration. `add_job()` rejects a new `at` job when an enabled job already exists for the same `(session_key, at_ms)` pair, returning `同一会话在 <time> 已存在一次性提醒 (id: …)，请勿重复创建；如需修改请先用 remove 删除旧任务，或改用其他时间`. The match is deliberately structural, not text-based — message wording is ignored, because the observed failure mode is the model re-adding the same reminder with reworded text within one turn, and each stored job would fire independently at the shared time. Different sessions may legitimately share a fire time, disabled jobs never block a fresh registration, and recurring schedules are exempt from this check. This complements the claim-before-dispatch guarantee below: that one stops a single job from firing twice across a restart; this one stops two jobs from existing for the same reminder in the first place.
