...
报给用户的字节数是实测 Content-Length：二进制 8,573,270（b5130 的 `whisper-bin-x64.zip`）、模型 `tiny` 77,691,713 / `base` 147,951,465 / `small` 487,601,967（base 与本机已下载文件逐字节相符）。未知档位返回 `null`，界面退化成"正在下载…"而不假装百分比。

这条按需下载与 `/api/ceo/transcribe` 同处一个无鉴权面（只有启动锁 423 闸门）：能访问该端口的人就能让这台机器去拉 157 MB，这与"能访问端口就能消耗本机 CPU"是同一条既有边界。
...
- `enabled`：默认 `True`。它是"允许使用"的许可，不是"已经装好"；关掉它之后，网页首点也不会触发下载。
...
- `language`：`auto`（正确性优先）或二进制语言表里的任一码（实测 100 条，`en`/`ja`/`ru`…，随多语种模型一起就在手里）。写错码不会报错：whisper 照常退出 0 而输出为空，本机信封把它说成 `stt_empty`（"没有识别到有效语音内容"），所以"突然全静音"式的故障要先核对这一格。
