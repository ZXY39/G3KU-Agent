...
The browser shell has a top-level `记忆管理` page. It is a read-only operator surface for the queued long-term memory runtime, plus two guarded mutation surfaces (failed-batch retry/discard and current-memory edit/delete) that stay inert unless `G3KU_ENABLE_MEMORY_ADMIN_MUTATIONS` is enabled.
...
- The page header's 查看记忆 button opens the memory browser drawer listing the current SQLite memory rows (创建时间, 刷新值, 通过次数, 来源, ID, 记忆内容). Sort toggles for 创建时间/刷新值/通过次数 and keyword search run browser-side over the fetched snapshot. The drawer header carries an 编辑 toggle next to 关闭: edit mode adds a leading selection column and a trailing 操作 column (per-row 修改 / 删除) plus a bulk bar (全选 checkbox, selected count, 删除选中); 修改 opens an in-drawer dialog for 记忆内容 and minimal_memory whose save asks for a second in-app confirmation, and both delete paths ask for a second confirmation before calling the backend. Outside edit mode the drawer shows no mutation controls.
...
- All mutation controls read their enabled state from the API (`mutations_enabled` on `/api/memory/failed` and `/api/memory/current`); without the server-side gate the buttons render disabled with a hint naming `G3KU_ENABLE_MEMORY_ADMIN_MUTATIONS`.
...
- `GET /api/memory/failed` returns parked failed records newest-parked-first with pagination metadata and `mutations_enabled`. `POST /api/memory/failed/{failed_id}/retry` requeues a parked record under its original `request_id`s at the queue tail; `POST /api/memory/failed/{failed_id}/discard` writes the `operator_discarded` terminal row into `ops.jsonl` and removes the parked record plus any stale queue rows. Missing records answer `404 memory_failed_not_found`, records not in `parked` state answer `409 memory_failed_not_parked`.
- `GET /api/memory/current` returns a snapshot of the current SQLite memory rows (memory id, body, source, refresh count, passed count, compression flag, timestamps) plus `mutations_enabled`, and backs the memory browser drawer. `POST /api/memory/current/update` edits one row (body required non-empty; candidate document is validated against the memory document limits before the SQLite write; `MEMORY.md` is rebuilt afterwards); `POST /api/memory/current/delete` bulk-deletes rows by id and reports `deleted` vs `missing`, rebuilding `MEMORY.md` once. Both append audit records like the other guarded mutations.
...
- Mutation controls render disabled when the API reports `mutations_enabled=false`; a missing retry/discard affordance with the env var unset is the intended gate, not broken DOM wiring.
