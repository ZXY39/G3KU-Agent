...
- The runtime does not auto-fill detached task parameters from this lane. If the model calls `create_async_task` for work that must reopen an uploaded file/image, it is expected to copy the relevant `path` / `ref` from `attachment_reopen_targets` into `create_async_task.task`.
