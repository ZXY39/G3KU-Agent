...
- The search input to the right of the 模型调用明细 title filters by 序号 (call sequence number, substring match), node id or model name (`model_key` / `provider_model` / `provider_id`); the filter runs over **all** records before pagination (searching never limits itself to the visible page), then pagination paginates the filtered set. Search and pagination rebuild only the table region (`refreshTaskTokenCallTable`), leaving the search input element, its focus and its value intact.
- The modal is `min(1200px, 100vw - 32px)` wide and the twelve-column table is compacted to fit it (0.78rem cells, 6px/7px padding) rather than being allowed to overflow: 节点ID and 模型 cells ellipsize in-cell with the full value in a `title` tooltip, so the right-most columns are not pushed out of view. `.task-token-call-table-wrap` keeps horizontal scrolling as the narrow-viewport fallback.
