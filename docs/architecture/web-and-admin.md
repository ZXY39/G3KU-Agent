...
- Persisted assistant messages keep projected cumulative `canonical_context` (`canonical_context_projection: stage_window`) and message-local `canonical_context_delta`; refresh and reconnect render the delta, while `preserved_turn` may use its archived full baseline.
