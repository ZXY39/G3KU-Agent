...
### CEO Snapshot Re-Render View State Contract

- `renderCeoSnapshot` is a full feed rebuild (message replay plus inflight/preserved turn restore) triggered by `snapshot.ceo` (reconnect/page load) and by `ceo.reply.final` closeouts that carry `user_messages`. A full rebuild clears the DOM, so it must preserve the operator's reading position instead of resetting it.
- Before rebuilding, the browser captures a transient per-session view state from the live DOM: the anchor element (`data-ceo-key`, with a bottom-relative positional fallback) plus its in-element scroll offset; per-turn Interaction Flow open state and `展开全部` history expansion; per-stage `<details>` open state (scoped per turn because `stage_index` fallback trace keys collide across turns); and the active round tool chip (scoped by per-turn DOM order because an empty round key would collide across rounds).
...
- Stable identity is mandatory for rebuilt feed nodes: messages get `data-ceo-key="m:{turn_id}:{role}:{occurrence}"`, turns get `data-ceo-key="turn:{turn_id}"`. New render paths that add feed nodes without these keys, or that change stage `traceKey` composition, silently weaken scroll anchoring and rebuild-stable expand state.
