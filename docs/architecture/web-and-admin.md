...
The left rail carries a `日志审计` page: a read-only, two-panel surface backed by a structured event sink — an `异常` panel listing the latest errors with their information source, and a `原始日志` terminal-style stream of every event. There is no per-subsystem classification or 24h rollup; sources are derived from each event's `subsystem` field as it is rendered.
...
- The `异常` panel fetches the newest `level=error` events (`limit=20`) and renders one row per exception — a source tag (模型调用 / 任务执行 / Web 接口 / 记忆处理, mapped from `subsystem`), the one-line summary, and the timestamp. The panel is hidden while no error events exist. Memory errors appear here read-only; their retry/discard surface stays in 记忆管理.
- The `原始日志` panel renders every event newest-first as one terminal-style line (timestamp | level | source | summary, monospace), with JSON-detail expansion per line and load-more pagination (page size 30). Both panels refresh every 15s while the view is active, loading full first and quietly afterwards like the memory page.
...
- The 异常 panel stays hidden although known errors exist: those events are not `level=error` or the view has not refreshed — compare `GET /api/audit/events?level=error` against the frontend request; the panel derives purely from that filter.
