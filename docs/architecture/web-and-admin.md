...
The left rail carries a `日志审计` page: a read-only, single-panel surface backed by a structured event sink — a `原始日志` terminal-style stream of every event, newest first, paginated at 100 events per page. The page keeps no per-subsystem classification, no 24h rollup, and no error-only surface: every event, errors included, lands in the one stream, and each row's source tag is derived from its `subsystem` field as it is rendered. `level` stays an API-side filter rather than a page control.
...
- The `原始日志` panel renders every event newest-first as one terminal-style line (system-local time | level | source | summary, monospace), with JSON-detail expansion per line. Timestamps are rendered from the stored offset-aware ISO value through `Date` into the operator machine's system-local `YYYY-MM-DD HH:mm:ss`; the raw ISO string stays in state untouched because the badge's `since` comparison is a server-side string comparison.
- The stream itself is the view's scroll container (`.audit-event-list` with `overflow-y: auto`), while the panel title bar and the pager footer stay pinned; the panel takes the remaining view height (`flex: 1 1 auto; min-height: 0`), so a 100-row page is reachable with the wheel instead of being clipped. Quiet 15s polls preserve the list scroll offset so reading older rows is not interrupted; paging and the explicit `刷新` reset the list to the top.
