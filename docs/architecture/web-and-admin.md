...
- `ConfigChatBackend` publishes live-only `model_retry_status` before each real retry — a retryable full-chain backoff and a same-model key rotation — and clears it when the retrying exits. A cross-model fallback is normal chain operation, so it is counted but does not itself emit a status. The status contains `state=retrying`, a `retry_count` equal to the number of provider requests actually sent so far (including rotations and cross-model fallbacks, hence equal to the ordinal of the retry about to happen), the chain round, the model refs, a bounded `error_message`, the backoff delay (0 for a rotation emission), and the absolute local-offset `last_retry_at` / `next_retry_at` timestamps; backend and frontend independently cap the error text so provider diagnostics cannot become oversized websocket/UI payloads. The task-node frame sanitizer (`_sanitize_model_retry_status`) is a field whitelist, so the two timestamps must be listed there explicitly or they are stripped before reaching the browser; the CEO snapshot deep-copies the whole status and needs no whitelist entry.
