...
- The web server runs one uvicorn event loop shared by every `/api/*` route and websocket. All `/api/tasks/*` read handlers (`main/api/rest.py`) are async and run their synchronous query work (SQLite reads, pydantic parsing, snapshot building) via `asyncio.to_thread`, so a heavy tree-snapshot or task-detail build cannot stall the light requests the task hall depends on (worker-status, list fetch, WS handshake). Control-plane handlers (pause/resume/cancel/delete/notice/bulk-delete) stay on the event loop.
