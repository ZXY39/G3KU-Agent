...
- Guarded operator mutations (`POST /api/memory/admin/retry-head`, the failed retry/discard pair, and the current-memory update/delete pair) are disabled by default; unless `G3KU_ENABLE_MEMORY_ADMIN_MUTATIONS` is enabled the backend returns `403` with `detail.code=memory_admin_mutation_disabled`, and successful calls append an audit record to `memory/admin_audit.jsonl`.
