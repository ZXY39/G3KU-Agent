...
- Each assistant turn renders a small per-response token line under the final bubble (`输入/缓存命中/输出 · 完成于 HH:MM:SS`), built from a normalized usage object `{input_tokens, output_tokens, cache_hit_tokens, call_count}` that maps directly onto normalized provider usage (`g3ku/providers/base.py`): `input_tokens` is the uncached lane, `cache_hit_tokens` the cache-read lane, `output_tokens` the generated output; no cost or cross-lane arithmetic is done in the browser. The completion time is the assistant transcript message's `timestamp` for history and the browser clock at closeout for a live turn.
...
- 模型调用明细 puts 序号（the call sequence number）first and defaults to 时间（`created_at`）descending with `call_index` descending as the same-instant tiebreak; records without a timestamp sink last. The table additionally shows 节点ID so per-node token attribution can be confirmed without leaving the window.
...
- The modal is deliberately wide (`min(1200px, 100vw - 32px)`) because the nine-column table must be readable without horizontal scrolling; long node ids are ellipsized in-cell with the full id in a `title` tooltip.
