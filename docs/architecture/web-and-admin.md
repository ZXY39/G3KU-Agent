...
- Node detail receives a backend-owned message list (its own section before `派生记录`) rather than a pseudo execution stage, and the frontend must not reconstruct entries from raw mailbox tables or prompt tail blocks. Distribution results are backend-owned and show both delivered targets (with the propagated message) and skipped targets (with the recorded reason). Each delivery row renders as a semantic label + icon chip via `messageDeliveryStatusDescriptor`/`renderMessageDeliveriesField` — 已分发·待处理 (`delivered`) / 已消费 (`consumed` without `merged_at`) / 已并入上下文 (`consumed` with `merged_at`) / 未下发 (`skipped`) — and must never surface raw ledger status tokens (`[delivered]`/`[consumed]`); the backend supplies `consumed_at`/`merged_at` on each distributed delivery for that derivation.
