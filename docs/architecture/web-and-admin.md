...
- Each settled assistant turn shows a small per-response token line under the final bubble (`收入/缓存命中/输出`), rendered from a normalized usage object `{input_tokens, output_tokens, cache_hit_tokens, call_count}`. The three counts map directly onto the normalized provider usage of `g3ku/providers/base.py`: `input_tokens` is the uncached input lane, `cache_hit_tokens` the cache-read lane, `output_tokens` the generated output; no cost or cross-lane arithmetic is done in the browser.
