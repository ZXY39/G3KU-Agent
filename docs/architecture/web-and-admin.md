...
- **Eligibility flag is server-owned.** `snapshot.ceo.messages[].can_edit_fork` (user items) is the sole render condition besides the `web:` prefix; assistant items may carry `task_dispatched`. The flag requires ALL of:
  - the session is a local `web:` session (channel sessions never get flags);
  - the session is fully stable — no inflight/preserved turn (including heartbeat/cron internal turns), no queued follow-ups, no pending approval interrupts, no blocking tool execution. The frontend mirrors this defensively: a feed-level `.ceo-turn-active` class hides every action row while `S.ceoTurnActive`, and clicks are guarded with a toast;
  - strict async-task gate: no verified `create_async_task` dispatch (`metadata.task_ids` on assistant transcript rows is authoritative) at or before the message — a task created in the clicked message's own reply turn also closes the gate. Gates walk the raw transcript (internal rows included); an assistant row with task ids retroactively closes every still-open user row of its group. Legacy transcripts without any `task_ids` field fall back to comparing `list_tasks_for_session` `created_at` against the run's first assistant reply timestamp;
  - the message is the first of its user-run (a maximal consecutive run of visible user messages — prompt-batch siblings and mid-turn consumed follow-ups share one execution turn and have no clean boundary, so only the run head is actionable; truncating the run head removes the whole run);
  - a per-turn boundary snapshot exists for the previous transcript message's turn (`.g3ku/web-ceo-turn-boundaries/<session>/<turn_id>.json.gz`, gzip, most recent 3 turns kept), or the boundary is the session start. There is no heuristic rebuild: messages older than the retained window are permanently ineligible.
...
- **Known edges**: bubbles appended by the live send path or client-side finalize carry no flag, so their action row appears only at the next authoritative snapshot (the render signature covers both flags, so the rebuild is guaranteed); a turn dispatched after the boundary is outside the gate by spec — truncation removes its transcript row while the task keeps running and its terminal callback still targets the session (banner text warns); the first post-edit request pays a one-time prompt-cache miss; the memory review window (`memory/review_state.json`) has no per-turn pruning, so buffered review records of removed turns may still flush to the advisory memory lane later (conversation context is unaffected); `user_edit_truncation` is a whitelisted continuity shrink/source reason (forensics in `context-and-cache-troubleshooting.md`).
...
- User bubbles and track-less assistant bubbles (the plain `addMsg` fallback) carry the same hover metadata through a `.msg-meta` line: `发送于` time for user messages, `完成于` time plus token usage for assistant fallbacks. The live send path stamps the browser clock onto the bubble and into the cached inflight `user_message.timestamp`; server snapshots supply the authoritative transcript `timestamp` on both roles.
...
- The CEO surface obeys the same protection. A `/ws/ceo` connect and `GET /api/ceo/sessions` build the session catalog across every CEO/channel transcript (a cold cache parses tens of MB of JSONL), load the requested session transcript, and project the transcript snapshot; all of it runs through `g3ku/runtime/ceo_catalog_offload.py` on one dedicated single-thread executor (`run_off_event_loop`), so a CEO WS reconnect — browser tabs wake from sleep and reconnect in bursts — never stalls task-hall endpoints. A short-TTL catalog cache collapses the repeated builds inside one connect/publish burst; write-side handlers (create/rename/activate/delete) store their fresh catalog through `store_ceo_catalog_cache`, whose generation bump discards any stale in-flight async build result. The executor thread reads the same `SessionManager` as the loop, so `SessionManager.get_or_create` guards its load path to keep a session from ever loading into two forked objects. New CEO read paths that walk transcripts must go through the same executor instead of the event loop.
...
- `_build_tree_snapshot` materializes only the current chunk's nodes and reads per-node state through the light projection rows (pause/handshake/pending-notice fields are written there at sync time); the runtime `nodes` blobs are only fetched for acceptance nodes, or batched once per build for legacy projection rows that predate the pending-notice field. Per-node full-payload `get_node` reads are what made whole-tree builds exceed the browser's 10s default timeout on large trees. Token aggregation likewise reads `payload_json` via `json_extract` instead of parsing whole detail payloads.
...
- A running compression renders `上下文压缩中` with a spinning indicator; a finished one renders `会话已压缩`; an interrupted one renders `压缩已暂停` with a pause icon. All three are the same `.ceo-compression-divider` row and use the `#39c5bb` accent (`--ceo-context-compress-color`). There is no separate composer toast — the feed rule replaced it.
- The running rule is live-only: it is appended while a compression is in flight and removed when it ends, and it is clickable — the spinning icon is a button that opens the shared in-page confirm dialog (`openConfirm`). Confirming calls `POST /api/ceo/sessions/{session_id}/compress-context/cancel`; cancelling the dialog changes nothing. The terminal rules are not clickable.
...
- The frontend learns a manual compression's terminal state by polling `GET /api/ceo/sessions/{session_id}/compress-context` until `status` leaves `running`, then reopens the session websocket so the persisted rule arrives with the transcript.
- During a manual compression the browser owns the visible running state, because an out-of-turn compression has no inflight turn to hang it on. Automatic compression keeps reading the snapshot's `inflight_turn.compression`; `state.compression` carries the same shape out of turn for other tabs.
...
- Pressing and holding the context-usage brain fills a circular ring around it; at 5 seconds the browser calls `POST /api/ceo/sessions/{session_id}/compress-context`. Releasing early resets the ring and starts nothing; the click that follows a completed hold is swallowed so it cannot also open the model panel. Touch pointers never arm the hold.
- Hovering the brain shows `长按压缩上下文` below the icon. This is the only hover text on the brain — usage figures still appear only inside the panel.
- The hold is refused, with a toast explaining why, on channel sessions, while an attachment upload is in flight, and while a compression already runs.
- The endpoint pauses first: when the session is running it awaits `session.pause(manual=True)` — which stops background tool executions and cancels in-flight requests — and only then starts compressing. Compression itself runs as a detached task, so the response returns immediately with `status: "running"`.
- A hold that finds nothing to compress (no compressible history, or a model without a usable context window) reports `status: "not_needed"` and writes no transcript rule.
...
- The headline is a single left-aligned pill carrying the display title of the model actually in use — `provider_model` resolved against the managed catalog (`name` > `key` > `provider_model`), falling back to the session's pinned binding title, and to `等待 Leader 上下文预估` when neither is known. The pill is muted (`is-pending`) while it shows a placeholder. Below it sit a ratio bar and an `estimated/total TOKEN` line rendered from the same estimate the brain uses (see `Composer Context Usage Meter`); the model name is never repeated under the bar, and that whole line is hidden when there is no estimate rather than echoing the placeholder a second time.
...
- With no data the base icon renders neutral with a faint glow while the colored fill stays `height 0` until an estimate arrives ("always lit" refers to the shell, not a fabricated fill). The shell keeps an `aria-label` of `model · estimated/total TOKEN` (`等待 Leader 上下文预估` when there is none) and no native `title`; the same numbers are surfaced once, in the panel. Hovering shows only the `长按压缩上下文` affordance hint, and a completed hold replaces the plain click with a fill ring plus a compression request (see `Manual Context Compression`).
