...
- 任务磁盘治理 UI（契约本体见 `runtime-overview.md`「磁盘写保护与治理」）：任务大厅性能条的磁盘数据与剩余空间合并显示在「CPU/内存/磁盘」一项的磁盘段（`0%(剩余10.1G)`，紧急=critical、清理线=throttled 着色随之落在该项上；数据来自 `GET /api/tasks/worker-status` 的 `machine_pressure_disk_busy_percent / machine_disk_free_bytes / disk_emergency_active / disk_cleanup_active`，1s 轮询），不再单列「磁盘剩余」项，紧急态在任务网格顶部渲染红色横幅。任务卡片动作只有 `暂停/恢复/删除`（kebab 菜单），删除确认文案明示「报告类产出自动导出到 deliverables 目录保留」。任务数据没有归档/压缩/固定状态——磁盘紧张时后端全删渐进直接彻底删除任务（先导产出），任务卡片经 WS `task.deleted` 推送从列表消失，无墓碑徽章；`.pc-corner-badge`、「已清理」chip、pin/compress/decompress 端点与按钮均不存在。`disk_usage_bytes` 仍在列表下发（每小时对账，展示延迟 ≤1h）。
