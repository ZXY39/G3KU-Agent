...
- `ConfigChatBackend` publishes live-only `model_retry_status` before each real retry (per-model backoff round or same-model key-switch resend), clearing when retrying exits. A cross-model fallback is normal chain operation, so it is counted but does not itself emit a status. The status contains `state=retrying`, a `retry_count` equal to the number of provider requests actually sent so far (including rotations and cross-model fallbacks, hence equal to the ordinal of the retry about to happen), the model retry round, the model refs, an `error_message` bounded only by a wide char limit (4096) that keeps websocket payloads from becoming oversized, the backoff delay (0 for a key-switch emission), and the absolute local-offset `last_retry_at` / `next_retry_at` timestamps; the task-node frame sanitizer (`_sanitize_model_retry_status`) is a field whitelist, so the two timestamps must be listed there explicitly or they are stripped before reaching the browser, and its error-text bound equals the emit-side limit because the browser expands the toast to the full received text and a tighter sanitizer bound would surface a cut-off expansion. The CEO snapshot deep-copies the whole status and needs no whitelist entry.
- For CEO sessions, the current running `inflight_turn.model_retry_status` reaches the browser through the existing `snapshot.ceo` / `ceo.turn.patch` lanes. The browser shows a small warning toast above the conversation feed while `state=retrying`, with text in the shape `模型自动重试中 · 第 N 次重试 · 最新 HH:MM:SS · 下次 HH:MM:SS · <error>` (the two clock fields render only when their timestamps are present); the toast disappears when the backend snapshot clears the field or the session switches to a non-running turn. The toast spans the content width of its container (the chat-column gutter in the session view, the drawer content column in node detail), and the frontend performs no char-level truncation of its own: long error text is visually clamped to two lines with an expand affordance, and clicking the toast (or Enter/Space while it is focused) expands it to the full received error text, with a second activation collapsing it again.
...
- Streamed text renders through a plain-text path while the turn runs; `ceo.reply.final` remains the authoritative closeout for the assistant bubble, where the full markdown render, transcript finalization, canonical-context finalization, and visible-turn completion happen.
...
- Each live patch merges its delta into the turn's already-rendered trace summary (stages and rounds merge by identity — appended or replaced, stage headers take the delta) instead of replacing the timeline, so completed stages stay visible while later phases render. A patch that carries no delta (no difference against the persisted baseline) is a no-op for the trace: the browser keeps the rendered stages and live tool steps instead of resetting the flow to the "waiting" placeholder. This preserves a mid-turn stage submission while the session stage state refreshes only at graph-node boundaries.
...
- `ceo.reply.final` canonical-context shape: the shared assembler `final_reply_canonical_merge` (`g3ku/runtime/web_ceo_sessions.py`), used by the userspace relay and the heartbeat publisher alike, includes the UI-projected `canonical_context` only when the completed turn has stage data and omits **both** `canonical_context` and `canonical_context_delta` when the turn added no new stage progress over the previous assistant message. The delta is computed with the same `ui_canonical_context_delta(...)` view, so a closeout never re-emits historical stages. Browsers prefer that payload over any older inflight or persisted trace; a message whose `canonical_context_delta` renders empty stages shows a plain text bubble with **no** stage rail, and neither `finalizeCeoTurn` nor `renderPersistedCeoAssistantTurn` may fall back to `turn.lastExecutionTraceSummary` or the session-cumulative `canonical_context`. Backfilling an older trace under a new direct-reply bubble is a contract bug.

### Per-Turn Token Usage Contract

- Each assistant turn renders a small per-response token line under the final bubble (`输入/缓存命中/输出`), built from a normalized usage object `{input_tokens, output_tokens, cache_hit_tokens, call_count}`. The three counts map directly onto the normalized provider usage of `g3ku/providers/base.py`: `input_tokens` is the uncached input lane, `cache_hit_tokens` the cache-read lane, `output_tokens` the generated output; no cost or cross-lane arithmetic is done in the browser.
- Visibility is state-dependent: while a turn is running the line is always visible and updates live; once the turn settles (finalized, paused, or loaded from history) the line is collapsed and revealed instantly on hover or keyboard focus of the bubble, so completed conversations stay uncluttered but the data is one pointer-motion away. A turn with no observable usage keeps the line hidden entirely.
- The usage object reaches the browser through the same CEO websocket lanes as the turn it describes: `snapshot.ceo.messages[].usage` for persisted history, `inflight_turn.usage` / `preserved_turn.usage` for live turn patches, and `ceo.reply.final.usage` for the authoritative closeout. The frontend may show an empty line when the turn produced no observable usage.
...
- The brain shell is always visible and hoverable; it never collapses to zero-opacity when an estimate is missing. With no data the base icon renders neutral with a faint glow, while the colored fill layer remains data-driven (`height 0` until an estimate arrives), so "always lit" refers to the shell, not to a fabricated fill.
- The meter exposes its numbers through a styled tooltip anchored above the icon (background, border, pop/expand animation on hover/focus), not the browser's native `title`. The tooltip shows `model · estimated/total TOKEN` when an estimate exists and a waiting label otherwise; the shell keeps a matching `aria-label` for accessibility.
