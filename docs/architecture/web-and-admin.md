...
- Display surfaces follow the binding `name` in live time: the session composer shows the active CEO config name left of the context-usage brain (`syncCeoActiveConfigLabel` reads `S.modelCatalog.defaults.ceo` and resolves the catalog title), and the task `Token统计` breakdown's per-config heading resolves `model_key` through the catalog to `name` (falling back to the key). Both redraw from `S.modelCatalog` state, so renames propagate on the next catalog refresh.
