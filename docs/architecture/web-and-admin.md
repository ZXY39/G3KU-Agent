...
- 模型调用明细 defaults to 时间（`created_at`）descending with `call_index` descending as the same-instant tiebreak; records without a timestamp sink last. The table additionally shows 节点ID so per-node token attribution can be confirmed without leaving the window.
...
- The search input to the right of the 模型调用明细 title filters by node id or model name (`model_key` / `provider_model` / `provider_id`); the filter runs over **all** records before pagination (searching never limits itself to the visible page), then pagination paginates the filtered set. Search and pagination rebuild only the table region (`refreshTaskTokenCallTable`), leaving the search input element, its focus and its value intact.
- The modal is deliberately wide (`min(1200px, 100vw - 32px)`) because the ten-column table must be readable without horizontal scrolling; long node ids are ellipsized in-cell with the full id in a `title` tooltip.
