...
The context-usage brain is also the composer's model-and-context entry: clicking it (or Enter/Space on it) opens an upward panel above the composer. The panel leads with the model actually in use and its context numbers, then the `模型链` / `指定模型` choice, then the matching pane. There is no separate model-mode button beside the brain, and no hover tooltip on the brain — the numbers live in the panel so they are not shown twice.

- The headline shows the current model's display title (`name` > `key` > `provider_model` resolved against the managed catalog) plus a `模型链` / `会话固定 · <展示名>` badge; below it sit a ratio bar and `model · estimated/total TOKEN` line rendered from the same estimate the brain uses (see `Composer Context Usage Meter`).
