...
- **消息三态**：message-list entries render `status` as 待处理 (`pending`：已投递、任何回合尚未处理) / 已消费 (`consumed`：控制或决策回合已处理，内容等待恢复路径并入或不再并入) / 已并入上下文 (`merged`：内容已真正注入模型上下文)。`messageListStatusDescriptor` 是三态标签的唯一映射；`consumed` 与 `merged` 的判定源是后端账本（`merged_at` 字段与归档记录），前端不做推断。
