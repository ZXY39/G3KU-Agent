...
The left rail carries a `日志审计` page: a read-only cross-subsystem health and error surface backed by a dedicated structured event sink. Scope rule: memory errors stay in 记忆管理 (`memory/failed.jsonl`) and never enter the audit system, so the audit page renders only `provider` / `task` / `web_api` events.
...
- The page renders three subsystem health cards (模型调用 / 任务执行 / Web 接口, zero-filled when empty) with 24h error/warning/event counts, a 正常/有错误 status, and the latest event's time + summary.
- The event feed is newest-first with level filter chips (全部/错误/警告/信息), load-more pagination (page size 30), JSON-detail expansion per card, and a 15s in-view auto-refresh that keeps scroll state; loading the view is full-then-quiet like the memory page.
- The audit view renders no memory surface elements; memory errors appear only in 记忆管理.
...
- Phase-1 emitter signal sources: `provider_chain_exhausted` (model-chain exhaustion, emitted once inside the `exhausted_model_chain_error` factory in `g3ku/providers/fallback.py`, covering both chain loops in `fallback.py` and `main/runtime/chat_backend.py`); `task_node_error` (from `TaskLogService.append_task_error_log`, the single chokepoint for all five task-node error writers); `task_terminal_failed` (from the terminal publish path, only when the final status is `failed`; success terminals stay out of the audit feed); `web_api_5xx` (an HTTP middleware in `g3ku/web/main.py` that captures ≥500 responses plus unhandled exceptions on `/api/` paths, with a `/api/audit` skip prefix so audit-endpoint failures never feed back into the pool). All emitters swallow their own exceptions; a broken sink never disturbs the host flow.
- `GET /api/audit/events` takes `limit` (1–200, default 50), `offset`, `level`, `subsystem`, `since` and returns `{ok, items, total, has_more}` newest-first; unknown filter values simply match nothing. `GET /api/audit/summary` returns `{ok, subsystems, generated_at}` with the three fixed subsystems always present. Read failures answer `503` with `detail.code` `audit_events_read_failed` / `audit_summary_read_failed` (Chinese messages in `api_client.js`).
...
- A failure class exists in the system but has no audit events: no emitter covers it yet — add one `emit_audit_event(...)` call at the failure's chokepoint (the recipe above); the fixed subsystem card list is cosmetic and needs no backend change for a new `subsystem` value.
- `/api/audit/*` returns 503: the sink read path cannot resolve a workspace root (`audit_sink_unconfigured`) or the file read failed; both endpoints share the audit read helpers in `g3ku/audit_events.py`.
- Memory events appearing in the audit feed is a regression: memory errors belong to `memory/failed.jsonl` and the 记忆管理 page, never to the audit pool.
