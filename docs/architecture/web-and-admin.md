...
- The note window (opened from note chips anywhere) shows the note body and carries an 编辑 toggle: edit mode swaps the body for a textarea whose save asks for a second in-app confirmation and posts to the note update endpoint. The note window exposes no delete control; note files are removed only through the memory-delete confirm dialog above or by `reconcile-notes`.
...
- Reading and writing the session setting goes through `GET` / `PATCH /api/ceo/sessions/{session_id}/model-selection`. The payload is `{mode: "chain"}` or `{mode: "model", model_key: "<binding key>"}`; unknown keys return 404 `model_key_not_found`, disabled keys 409 `model_key_disabled`, model mode without a key 400 `model_key_required`, and channel sessions 409 `channel_session_readonly`.
- The setting is stored in the local session's metadata (`model_selection`) and persists across restart; it is not exposed on channel sessions, where the composer is read-only and the panel never opens.
