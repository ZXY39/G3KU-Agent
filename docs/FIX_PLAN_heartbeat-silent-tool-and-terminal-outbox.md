...
## 7. 上线后追加段（2026-09-23 23:25 实盘）

P0–P6 于 23:15:01 随进程重启生效（判据：实盘契约行出现 `` `silent` ``，同会话重启前三轮没有）。当晚 23:25 暴露一处计划外缺口：

- **现象**：QQ 会话 `ext:qq-official:f8a8001865631301`（turn `e42cc4fc95f44443`）用户说"读桌面文件数，然后等我来问你"，模型 `exec` 取到 `50` 后收尾；23:25:51 `qq_official.bridge:deliver` 把 `No visible reply was generated for: 接下来读取桌面文件数…` 投给了用户，网页也没折叠。
- **根因**：`_graph_finalize_turn` 的空回复兜底（`_ceo_support.py` 的 `_empty_reply_fallback`，3.82 遗留）把"没有正文"当"要回复"。而模型给的正文按它上下文里残留的旧契约就是纯 `[G3KU_SILENT]` —— P4 的清洗把它剥成空，正好落进兜底。**两个缺陷都是"机器替模型决定推送"**，与 §2.0 裁决同向。
- **为什么模型没调工具**：`silent` 的措辞当时只存在于心跳车道（`heartbeat/session_service.py` 四处），可见用户回合从未被告知这个出口；同时 durable 压缩块 `.g3ku/web-ceo-continuity/ext_qq-official_f8a8001865631301.json` → `frontdoor_request_body_messages[1]` 里那句「需静默时整条回复只输出 `[G3KU_SILENT]`」每轮重注入。
- **A（`f1af191e`）**：可见回合空正文 → `silent_reply=true`，英文兜底整条删除；纯哨兵与真空正文两种成因在 `silent_reason` 上分开留痕。附带纠正异步派发恢复处沿用的旧形态（痕迹行 `prompt_visible=False` + 空行），两个收尾点共用一份元数据构造。
- **B（`cee575f1`）**：契约在 `silent` 进入 callable 名单时渲染一行 `silent_help:`，并明说旧哨兵不是静默出口。实测该契约 6,614 字符，这条 +215 ≈ 每轮 +55 token，不新起一层。
- **C（未做，属操作员）**：压缩块里那句旧契约措辞。机器不改 durable 基线（字节对账会清 trace）；做法是在该会话发一条可见消息纠正后手动压缩一次。A 已使这句旧措辞不造成后果。
- **口径影响**：§6 风险 3 的"渠道被静默则屏幕上什么都没有"由 A 扩大到"模型不给正文也算"。接受，理由同 §2.0：假文案不是内容，一条无意义英文与一条中文提示都不构成"用户收到了该收到的东西"。
