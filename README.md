...
G3KU 是一套面向复杂工作流的 Harness。一个能自进化，能扩展、能推进复杂工作、能接入外部通信渠道、支持 Web 管理界面的智能工作系统。
...
7. 🛡️ **安全机制**
   系统具备权限控制、人工审批、风险隔离和敏感信息保护，适配更真实的业务环境。
...
为了实现对话功能，至少需要先给主 Agent配置一个模型。但是确保功能正常运行，需要为：
...
如果模型支持 OpenAI 协议兼容接口，优先建议选择下面两类供应商：
