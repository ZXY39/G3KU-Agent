...
```powershell
iwr https://raw.githubusercontent.com/ZXY39/G3KU-Agent/v1.0.4/install.ps1 | iex
```
...
iwr https://cdn.jsdelivr.net/gh/ZXY39/G3KU-Agent@v1.0.4/install.ps1 | iex
iwr https://ghfast.top/https://raw.githubusercontent.com/ZXY39/G3KU-Agent/v1.0.4/install.ps1 | iex
```

自 `v1.0.2` 起每个发布版本都带 release 资产，可绕开 raw 域名：`https://github.com/ZXY39/G3KU-Agent/releases/download/v1.0.4/install.ps1`。第三方代理等于把"执行什么代码"交给它，稳妥做法是先下载核对再运行 —— `install.ps1` 当前 7483 字节、`install.sh` 5976 字节，且首行是 `#requires -Version 5.1` / `#!/usr/bin/env bash`。
...
- 升级：`.\install.ps1 -Upgrade`（Linux / macOS `./install.sh --upgrade`）。默认升到脚本里钉住的 ref，要指定版本加 `-Ref v1.0.4` / `--ref v1.0.4`。
