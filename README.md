...
当前系统要求至少给主 Agent，也就是 CEO 角色分配一个可用模型。若没有配置模型，Web 会提示你先进入“模型配置”页面，新增并保存至少一个模型，再把它分配给主 Agent（CEO）角色。

## 3. 如何配置模型

模型配置推荐直接在前端完成，普通用户不需要先手动编辑大量 JSON 文件。
...
2. 先点击“新建配置”。
3. 新建好模型配置后，保存模型。
4. 再把左侧模型拖动到右侧对应 Agent 的模型链中。
5. 完成后点击“保存模型链”。

你至少应该先给主 Agent，也就是 `CEO` 角色配置一个模型。常见情况下，也可以继续为：
...
如果你用的是 OpenAI 协议兼容服务，优先建议选择下面两类供应商模板：

- `Custom OpenAI-Compatible`
- `OpenAI Responses`

这样更容易对接兼容 OpenAI 协议的模型服务。
...
如果你需要知识检索、记忆增强或更强的语义检索效果，再去配置 RAG 会更合适。
