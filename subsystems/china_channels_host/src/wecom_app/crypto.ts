 ...
 * 计算企业微信消息签名
 */
export function computeWecomAppMsgSignature(params: {
  token: string;
  timestamp: string;
  nonce: string;
  encrypt: string;
 ...
 * 验证企业微信签名
 */
export function verifyWecomAppSignature(params: {
  token: string;
  timestamp: string;
  nonce: string;
  encrypt: string;
 ...
 * 解密企业微信消息
 */
export function decryptWecomAppEncrypted(params: {
 ...
 * 加密企业微信回复消息
 */
export function encryptWecomAppPlaintext(params: {
  encodingAESKey: string;
  receiveId?: string;
  plaintext: string;
}): string {
