...
def test_handle_create_save_uses_current_dom_json_and_model_key() -> None:
    ...
    assert result["createPayload"]["binding"]["image_multimodal_enabled"] is True
    assert "image_multimodal_enabled" not in result["createPayload"]["draft"]["parameters"]
    assert "imageMultimodalEnabled" not in result["createPayload"]["draft"]["parameters"]


def test_handle_create_save_closes_editor_before_runtime_refresh_polling() -> None:
    result = _run_node_script(
        """
        const fs = require("fs");
        const vm = require("vm");
        ...
        global.window = global;
        global.window.addEventListener = () => {};
        global.window.setTimeout = (fn, _ms) => { fn(); return 1; };
        ...
        global.U = {};
        let editorOpenWhenPolled = null;
        global.ApiClient = {
          validateLlmDraft: async () => ({ valid: true }),
          probeLlmDraft: async () => ({ success: true, message: "ok" }),
          createLlmBinding: async (payload) => ({
            item: payload.binding,
            runtimeRefresh: {
              worker_refresh_requested: true,
              worker_refresh_acked: false,
              worker_refresh_command_id: "command:create-1",
              worker_refresh_status: "pending",
            },
          }),
          getRuntimeRefreshStatus: async () => {
            editorOpenWhenPolled = Boolean(global.S.llmCenter.editor?.open);
        ...
        (async () => {
          await window.__llmTestHooks.handleCreateSave();
          console.log(JSON.stringify({ editorOpenWhenPolled }));
        })().catch((error) => {
          console.log(JSON.stringify({ error: error.message || String(error), editorOpenWhenPolled }));
          process.exit(1);
        });
        """
    )

    assert result["editorOpenWhenPolled"] is False
...
def test_handle_detail_save_closes_editor_before_runtime_refresh_polling() -> None:
    result = _run_node_script(
        """
        const fs = require("fs");
        const vm = require("vm");
        ...
        const changedJson = JSON.stringify({
          ...
          auth_mode: "api_key",
          api_key: "detail-key",
          default_model: "detail-model-updated",
          parameters: { context_window_tokens: 32000 },
          extra_headers: {},
          extra_options: {},
        });
        ...
        global.ApiClient = {
          ...
          updateLlmConfig: async (_configId, payload) => ({
            ...
            runtimeRefresh: {
              worker_refresh_requested: true,
              worker_refresh_acked: false,
              worker_refresh_command_id: "command:update-1",
              worker_refresh_status: "pending",
            },
          }),
          updateLlmBinding: async () => ({ item: {}, runtimeRefresh: null }),
          getRuntimeRefreshStatus: async () => {
            editorOpenWhenPolled = Boolean(global.S.llmCenter.editor?.open);
            return { status: "completed" };
          },
...
def test_handle_detail_save_updates_binding_image_multimodal_flag_without_polluting_json_draft() -> None:
    result = _run_node_script(
        """
        const fs = require("fs");
        const vm = require("vm");
        ...
        const initialJson = JSON.stringify({
          ...
          api_key: "detail-key",
          default_model: "detail-model",
          parameters: { context_window_tokens: 32000, temperature: 0.2 },
          extra_headers: {},
          extra_options: {},
        });
        const elements = {
          "llm-json-editor": { value: initialJson },
          "llm-binding-retry-on": { value: "network,429,5xx" },
          "llm-binding-retry-count": { value: "0" },
          "llm-binding-single-api-key-max-concurrency": { value: "" },
          "llm-binding-context-window-tokens": { value: "32000" },
        ...
        (async () => {
          ...
          console.log(JSON.stringify({ error: error.message || String(error), updateConfigPayload, updateBindingPayload }));
          process.exit(1);
        });
        """
    )

    assert result["updateConfigPayload"] is None
    assert result["updateBindingPayload"]["image_multimodal_enabled"] is True


def test_handle_detail_save_persists_context_window_tokens_when_only_input_changes() -> None:
    result = _run_node_script(
        """
        const fs = require("fs");
        const vm = require("vm");
        ...
        const elements = {
          ...
          "llm-binding-retry-count": { value: "0" },
          "llm-binding-single-api-key-max-concurrency": { value: "" },
          "llm-binding-context-window-tokens": { value: "36001" },
          "llm-bindings-list": { innerHTML: "", addEventListener: () => {} },
          "llm-editor-shell": { innerHTML: "", addEventListener: () => {} },
          "llm-editor-backdrop": { addEventListener: () => {} },
          "llm-memory-settings-btn": { addEventListener: () => {} },
        ...
        (async () => {
          await window.__llmTestHooks.handleDetailSave();
          console.log(JSON.stringify({ updateConfigPayload }));
        })().catch((error) => {
          console.log(JSON.stringify({ error: error.message || String(error), updateConfigPayload }));
          process.exit(1);
        });
        """
    )

    assert result["updateConfigPayload"]["parameters"]["context_window_tokens"] == 36001


def test_handle_detail_save_respects_json_context_window_tokens_when_json_changes_only() -> None:
    result = _run_node_script(
        """
        const fs = require("fs");
        const vm = require("vm");
        ...
        const changedJson = JSON.stringify({
          ...
          api_key: "detail-key",
          default_model: "detail-model",
          parameters: { context_window_tokens: 36555 },
          extra_headers: {},
          extra_options: {},
        });
        const elements = {
