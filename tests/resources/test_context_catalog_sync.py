...
async def test_context_catalog_indexer_rebuilds_tool_record_when_only_metadata_changes(
    ...
    service = SimpleNamespace(
        current_family=SimpleNamespace(
            ...
            actions=[SimpleNamespace(executor_names=["demo_tool_exec"])],
        ),
        current_toolskill={
            "tool_id": "demo_tool_exec",
            "description": "Old tool summary",
            "content": tool_body,
            "path": "",
        },
    ...
    record = memory_manager.records["tool:demo_tool_exec"]
    assert first == {"created": 1, "updated": 0, "removed": 0}
    assert second == {"created": 0, "updated": 1, "removed": 0}
    assert "New tool title" in record.l0
    assert "New tool summary" in record.l1


@pytest.mark.asyncio
async def test_context_catalog_indexer_writes_one_record_per_concrete_tool_executor(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _skip_model_summary(*args, **kwargs):
        _ = args, kwargs
    ...
    service = SimpleNamespace(
        ...
        list_tool_resources=lambda: [
            SimpleNamespace(
                ...
                description="Filesystem family.",
                actions=[
                    SimpleNamespace(executor_names=["filesystem_copy", "filesystem_write"]),
                ],
            )
        ],
        get_tool_toolskill=lambda tool_id: dict(toolskills[str(tool_id)]),
    )
    memory_manager = _MemoryManager()
    indexer = ContextCatalogIndexer(memory_manager=memory_manager, service=service)

    result = await indexer.sync(tool_ids={"filesystem"})

    assert result == {"created": 2, "updated": 0, "removed": 0}
    assert sorted(memory_manager.records.keys()) == [
