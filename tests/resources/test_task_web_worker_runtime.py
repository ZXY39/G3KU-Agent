...
def _stamp_incomplete_spawn_round(
    service: MainRuntimeService,
    *,
    root_node_id: str,
    round_id: str,
...
def test_node_actual_request_artifact_falls_back_to_preflight_truth_when_usage_has_no_input_tokens(
    tmp_path: Path,
) -> None:
    service = MainRuntimeService(
        chat_backend=_DummyChatBackend(),
        ...
        lambda frame: {
            **frame,
            "token_preflight_diagnostics": {
                "final_request_tokens": 4321,
                "provider_model": "test:gpt-5.4",
...
async def test_distribution_barrier_preempts_waiting_children_recovery(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    try:
        ...
        # owner 三项必须与下方 spawn_operations 轮一致：否则该轮按 runtime 口径仍是
        # 「未物化轮」，命中未物化豁免，屏障就不拦阻重放（见新契约用例）。
        child = service.node_runner._create_execution_child(
            task=task,
            parent=root,
            spec=spec,
            owner_round_id="call:spawn-round",
            owner_entry_index=0,
        )
        assert child is not None
...
@pytest.mark.asyncio
async def test_resume_ready_pending_notice_preempts_waiting_children_recovery(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    service = MainRuntimeService(
    ...
    try:
        ...
        # owner 三项必须与下方 spawn_operations 轮一致，理由同 barrier 用例。
        child = service.node_runner._create_execution_child(
            task=task,
            parent=root,
            spec=spec,
...
@pytest.mark.asyncio
async def test_unmaterialized_spawn_round_bypasses_distribution_block(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    """未物化轮优先于屏障拦阻：drain 等的就是这一轮的物化，拦死即互等。"""
    service = MainRuntimeService(
        chat_backend=_DummyChatBackend(),
        workspace_root=tmp_path,
        store_path=tmp_path / "runtime.sqlite3",
    ...
    try:
        ...
        # 故意不绑定 owner 三项：entry 虽已写 child_node_id，但该子节点不归本轮所有，
        # 按 spawn_round_has_unmaterialized_entries 的口径仍是「未物化」。
        child = service.node_runner._create_execution_child(
            task=task,
            parent=root,
            spec=spec,
...
@pytest.mark.asyncio
async def test_resume_react_state_keeps_restored_frame_when_pending_notice_waits_for_children(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    service = MainRuntimeService(
    ...
    try:
        record = await service.create_task("hold notice until children finish", session_id="web:shared")
        task = service.get_task(record.task_id)
        root = service.get_node(record.root_node_id)
        ...
        service.log_service.upsert_frame(
            record.task_id,
            {
                **service.log_service._default_frame(
                ...
                "stage_status": "active",
                "stage_goal": "keep existing round",
                "messages": [
                    {"role": "system", "content": "system seed"},
                    {"role": "user", "content": "historic request"},
                ],
        ...
        react_state = await service.node_runner._resume_react_state(task=task, node=root)

        frame = service.log_service.read_runtime_frame(record.task_id, root.node_id) or {}
        root_after = service.get_node(root.node_id)
        pending_notice_state = dict((root_after.metadata or {}).get(PENDING_NOTICE_STATE_KEY) or {})

        assert react_state["message_source"] == "restored_frame"
        assert react_state["messages"] == [
            {"role": "system", "content": "system seed"},
            {"role": "user", "content": "historic request"},
        ]
        assert list(react_state.get("pending_root_notice_ids") or []) == []
        assert frame.get("stage_status") == "active"
        assert frame.get("stage_goal") == "keep existing round"
        assert list((root_after.metadata or {}).get("pending_append_notice_records") or [])
        assert pending_notice_state["resume_mode"] == RESUME_MODE_WAIT_FOR_CHILDREN
        assert pending_notice_state["holding_round_id"] == "call:spawn-round"
    finally:
        await service.close()


@pytest.mark.asyncio
async def test_resume_ready_pending_notice_wait_for_children_allows_waiting_children_recovery(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    service = MainRuntimeService(
...
async def test_resume_react_state_injects_pending_notice_once_active_round_is_gone(
    ...
    try:
        record = await service.create_task("consume held notice after child round", session_id="web:shared")
        task = service.get_task(record.task_id)
        root = service.get_node(record.root_node_id)
        ...
        service.log_service.upsert_frame(
            record.task_id,
            {
                **service.log_service._default_frame(
                ...
                "stage_status": "active",
                "stage_goal": "consume held notice",
                "messages": [
                    {"role": "system", "content": "system seed"},
                    {"role": "user", "content": "historic request"},
                ],
        ...
        assert react_state["message_source"] == "notice"
        assert react_state["messages"][-1]["role"] == "user"
        assert react_state["messages"][-1]["content"] == "new constraint"
        assert react_state["pending_root_notice_ids"] == ["root-notice:demo:1"]
...
async def test_resume_ready_pending_notice_preempts_pending_tool_turn(tmp_path: Path):
    ...
    try:
        record = await service.create_task("recover pending tool turn", session_id="web:shared")
        task = service.get_task(record.task_id)
        root = service.get_node(record.root_node_id)
            ...
            tool_calls=[
                {
                    ...
                    "arguments": {
                        "path": str(tmp_path / "write-target.txt"),
                        "content": "stale body",
                    },
                }
            ],
            live_tool_calls=[{"tool_call_id": "call:write", "tool_name": "filesystem_write", "status": "running"}],
            content="pending tool turn before notice consumption",
        )
        service.log_service.update_node_metadata(
            root.node_id,
            lambda metadata: {
...
@pytest.mark.asyncio
async def test_task_snapshot_payload_with_node_detail_includes_spawn_review_rounds(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    backend = _SpawnReviewToolCallChatBackend(
        arguments={
            "allowed_indexes": [1],
            "blocked_specs": [
