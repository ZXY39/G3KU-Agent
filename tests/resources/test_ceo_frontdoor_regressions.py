...
def _ai_message_from_response(response: LLMResponse) -> AIMessage:
    return AIMessage(
...
def _patch_langgraph_model_calls(monkeypatch, runner: LangGraphCeoRunner, responses: list[LLMResponse], sink: list[dict[str, object]]):
        ...
        sink.append(
            {
                ...
                "langchain_tools": list(langchain_tools or []),
                "model_refs": list(model_refs or []),
                "parallel_tool_calls": parallel_tool_calls,
                "prompt_cache_key": prompt_cache_key,
            }
        )
        return _ai_message_from_response(responses.pop(0))
...
@pytest.mark.asyncio
async def test_langgraph_ceo_runner_directly_executes_visible_tool_without_stage(monkeypatch, tmp_path) -> None:
    async def _noop_ready() -> None:
        return None
    ...
    runner = LangGraphCeoRunner(loop=loop)
    ...
    monkeypatch.setattr(runner._builder, "build_for_ceo", _build_for_ceo)
    monkeypatch.setattr(runner, "_resolve_ceo_model_refs", lambda: ["openai_codex:gpt-test"])
    _patch_langgraph_model_calls(monkeypatch, runner, responses, model_calls)
...
@pytest.mark.asyncio
async def test_langgraph_ceo_runner_does_not_duplicate_current_user_when_builder_already_includes_it(monkeypatch, tmp_path) -> None:
    async def _noop_ready() -> None:
        return None
    ...
    assert output == "done"
    messages = list(model_calls[0]["messages"] or [])
    assert [str(item.get("content") or "") for item in messages].count("follow up") == 1


@pytest.mark.asyncio
async def test_langgraph_ceo_runner_executes_xml_tool_call_directly_without_repair(monkeypatch, tmp_path) -> None:
    async def _noop_ready() -> None:
        return None
    ...
    assert not any(
        "XML-style pseudo tool calling" in str(item.get("content") or "")
        for item in list(model_calls[1].get("messages") or [])
    )


@pytest.mark.asyncio
async def test_langgraph_ceo_runner_retries_empty_turn_until_valid_result(monkeypatch, tmp_path) -> None:
    async def _noop_ready() -> None:
        return None
    ...
    monkeypatch.setattr("g3ku.runtime.langgraph_ceo.runner.asyncio.sleep", _fake_sleep)
