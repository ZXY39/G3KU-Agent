...
def test_task_message_distribution_storage_round_trip(tmp_path: Path) -> None:
    ...
    try:
        ...
        fetched = store.get_task_message_distribution_epoch(record.epoch_id)
...
def test_task_node_notification_storage_round_trip(tmp_path: Path) -> None:
    ...
    try:
        ...
        task_notifications = store.list_task_node_notifications(notification.task_id)
        ...
        assert task_notifications[0].status == "queued"
