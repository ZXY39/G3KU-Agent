...
- 若当前节点上下文提供了 `runtime_environment.task_temp_dir`，临时内容必须写到该任务级临时目录下。
- 若没有提供 `runtime_environment.task_temp_dir`，临时内容才回退到工作区根目录的 `temp/` 下。
- 这里的“临时内容”包括下载结果、抓取缓存、一次性脚本、日志、解压产物、测试输出、临时生成文件和其他不应长期留在正式目录中的内容。
- 只有为了满足任务要求且只能写到其他目录时，才允许例外；例外时必须显式使用绝对路径，不得隐式落到项目根目录。
- 第三方工具本体、解压后的可执行文件、运行时、模型文件和附带依赖，必须写到工作区根目录的 `externaltools/<tool_id>/` 下。
- `tools/` 目录只用于注册和适配，不用于存放第三方工具本体。
- 在 `tools/<tool_id>/` 下只应保留 `resource.yaml`、`main/`、`toolskills/` 及其注册代码、包装脚本和说明文档。
...
- 要落盘前先判断文件属于哪一类：
  ...
  - 临时内容：写到 `runtime_environment.task_temp_dir`；没有该字段时回退到 `temp/`。
  - 第三方工具本体：写到 `externaltools/<tool_id>/`。
- 如果一个路径看起来像压缩包、安装包、二进制、日志、缓存或其他临时产物，但不在任务级临时目录或 `externaltools/` 下，应改路径后再执行。

## 常见例子

- 临时抓取结果：`<task_temp_dir>/fetch/result.json`
- 临时脚本：`<task_temp_dir>/scripts/check.ps1`
- 没有任务级目录时的临时脚本回退：`<workspace>/temp/scripts/check.ps1`
- 第三方工具安装目录：`<workspace>/externaltools/ffmpeg/`
- 工具注册文件：`<workspace>/tools/ffmpeg/resource.yaml`

## 先停一下再继续的情况

- 你准备把下载包、压缩包、日志、缓存或测试输出写到任务级临时目录或 `temp/` 之外。
