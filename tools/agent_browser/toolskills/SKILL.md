...
### 推荐核心工作流（基于 0.26.0 官方 core skill）

- 先 `open <url>` 打开页面。
- 立即 `snapshot -i` 获取交互元素快照与 `@eN` refs。
- 后续点击、输入、选择等操作优先使用 `click @eN`、`fill @eN`、`type @eN`。
- 页面一旦发生变化（导航、重渲染、弹层、提交表单），必须重新执行 `snapshot -i`，旧 refs 会立刻失效。
- 读取页面信息时，优先使用新版本正式命令：
  - `get title`
  - `get url`
  - `get text @eN`
  - `get attr @eN href`
- 如果不想先 snapshot，也可以使用 `find role/text/label/...` 这类语义定位器；原始 CSS 选择器应作为后备方案。

## 重要语义与预期管理

- 这个工具是 **官方 `agent-browser` CLI 的自动化包装器**，优先语义是“执行浏览器自动化会话中的某一步命令”，**不是**“像桌面助手一样永久打开一个本地浏览器窗口并保持不动”。
- 按官方 0.26.0 core skill，浏览器会在多个命令之间持续运行，形成同一 session；因此 `open -> snapshot -> click/fill -> get` 是正常工作流。
- 但在 g3ku 包装层里，如果命令超时、会话清理被触发，或平台主动回收执行，就不能把状态转述成“页面仍稳定常驻打开”。
- 当用户明确要求“打开后保持页面不动 / 让我持续看到这个页面”时，必须区分：
  - `agent-browser` 能否维持自动化 session；
  - 平台本轮执行是否已经结束或触发清理。
  若不能保证持续可见，必须明确说明限制。
