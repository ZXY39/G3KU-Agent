...
- 每个阶段都必须提供清晰的 `stage_goal` 和 1 到 15 的 `tool_round_budget`。
...
- 判定契约（注意与常规验收语义相反，以本模式为准）：
  - **阻塞成立** → `success` + `delivery_status="final"`，`evidence` 必须至少一条，引用你实际核验过的证据；无证据的 success 会被系统判为无效裁决并要求重验。
  - **阻塞不成立** → `failed` + `delivery_status="final"`，`blocking_reason` 写明执行节点接下来必须做什么；系统会把执行节点打回继续处理。
  - **核验无法完成** → `failed` + `delivery_status="blocked"`，`blocking_reason` 写明原因。
- 任何 `failed` 结论都不允许用于占位或敷衍；同样，执行节点也不允许用 `failed+blocked` 占位——你的职责就是把这类占位识别并打回。
