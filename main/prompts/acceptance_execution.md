...
- 若下一阶段不再需要本阶段的原始工具入参/出参，就在同一次提交里带上 `drop_completed_stage_tool_detail: true`：本阶段的工具肉身从此不再进入上下文，只留下你刚写的 `completed_stage_summary`。它要求该总结非空，否则判参数非法。移出的是上下文不是数据——完整轨迹仍在任务里，事后用 `task_node_detail` 回读；阶段块上带 `evicted` 字段的，就是你主动移走的那条。
...
- 任何一回合只要没有工具调用、只输出纯文本，都不构成交付：运行时不会把这段文字包装成验收结论，而是打回并附阶段协议提醒后让你继续。要继续核验就调用普通工具，要裁定就直接调用 `submit_final_result`，需要新一轮核验就同批提交 `submit_next_stage` + 目标工具。连续 3 回合只回纯文本会被判 `failed + blocked` 收口并转入错误暂停，那不是正常收尾。
