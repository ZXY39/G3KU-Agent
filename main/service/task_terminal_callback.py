...
def build_task_terminal_payload(task: Any) -> dict[str, str]:
    ...
    payload = {
        ...
        'continuation_state': _normalize_task_terminal_text((getattr(task, 'metadata', None) or {}).get('continuation_state')).lower(),
        'continuation_mode': _normalize_task_terminal_text((getattr(task, 'metadata', None) or {}).get('continuation_mode')).lower(),
        'continued_by_task_id': _normalize_task_terminal_text((getattr(task, 'metadata', None) or {}).get('continued_by_task_id')),
...
def normalize_task_terminal_payload(payload: dict[str, Any] | None) -> dict[str, str]:
    ...
    return {
        ...
        'continuation_state': _normalize_task_terminal_text(source.get('continuation_state') or source.get('continuationState')).lower(),
        'continuation_mode': _normalize_task_terminal_text(source.get('continuation_mode') or source.get('continuationMode')).lower(),
        'continued_by_task_id': _normalize_task_terminal_text(source.get('continued_by_task_id') or source.get('continuedByTaskId')),
