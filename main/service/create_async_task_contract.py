...
CREATE_ASYNC_TASK_CONTINUATION_OF_TASK_ID_DESCRIPTION = (
    '若这是对某个已失败任务的续跑，请传入原失败任务 id，格式为 task:xxxx。'
)

CREATE_ASYNC_TASK_REUSE_EXISTING_DESCRIPTION = (
    '是否优先复用同一会话下、针对同一个 continuation_of_task_id 的进行中续跑任务。默认 true。'
)
